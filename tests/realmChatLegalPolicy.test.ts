...
import { REALM_CHAT_SERVER_ACTIVATION_ALLOWED } from '../spacetimedb/src/realmChatPolicy';
...
describe('disabled Realm Chat legal contract', () => {
  it('keeps the proposed social feature unavailable pending explicit review and activation', () => {
    ...
    expect(REALM_CHAT_SERVER_ACTIVATION_ALLOWED).toBe(false);
    expect(WARPKEEP_ENTRY_AGREEMENT_VERSION)
      .toBe('2026-07-31-hegemony-entry-agreement-v4');
    expect(WARPKEEP_ENTRY_AGREEMENT_RELEASE_STATUS).toBe('production-approved');
  });
});
