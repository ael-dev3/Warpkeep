...
const MANIFEST_PATH = resolve(
  ROOT,
  'docs/reference/resources/2026-07-18-hegemony-logging-camp/record-art/manifest.json'
);
const manifest = JSON.parse(readFileSync(MANIFEST_PATH, 'utf8')) as {
  ...
  runtimeAsset: {
    ...
    brightNeutralPartialAlphaAudit: {
      pixels: number;
      maximumAllowedPixels: number;
    };
...
function brightNeutralPartialAlphaPixels(raw: Buffer) {
  let count = 0;
...
describe('Hegemony Logging Camp record art', () => {
  it('pins the narrow visual authorization without promoting a Logging Camp into gameplay', () => {
    expect(manifest.schemaVersion).toBe(1);
    expect(manifest.projectAuthorization.scope).toContain('PR #62');
    ...
    expect(manifest.projectAuthorization.notGranted.join(' ')).toMatch(
      /Wood.*(?:placement|renderer|server|balance|reward|entitlement).*authority/i
    );
    ...
    expect(manifest.presentationBoundary.currentNodePlacement)
      .toBe('validated public Wood-site inspector only');
    ...
    expect(attribution).toContain('Hegemony Logging Camp inspection artwork');
  ...
  it('ships one exact bounded alpha WebP with transparent corners and clean margins', async () => {
    ...
    expect(brightNeutralPartialAlphaPixels(raw))
      .toBe(manifest.runtimeAsset.brightNeutralPartialAlphaAudit.pixels);
    expect(manifest.runtimeAsset.brightNeutralPartialAlphaAudit.pixels)
      .toBeLessThanOrEqual(manifest.runtimeAsset.brightNeutralPartialAlphaAudit.maximumAllowedPixels);
  ...
  it('keeps the asset local and decorative in the validated Wood inspection panel', () => {
    ...
    expect(component).toContain("publicAssetUrl('images/realm/hegemony-logging-camp-record.webp')");
    expect(component).toContain('logging-camp-inspection__art-stage');
    expect(component).toContain('logging-camp-inspection__hero-art');
