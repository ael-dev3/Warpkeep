...
describe('semantic realm terrain features', () => {
  ...
  it('ignores historical heath blooms and presents ancient stone as neutral moss-slate', () => {
    const { placements, semantics, surface } = canonicalInput();
    const generated = generateRealmTerrainFeatures(
      surface.renderMap,
      semantics.terrainKindsByKey,
      'high',
      1,
    ...
    const historicalBloom = Object.freeze({
      kind: 'heath-bloom' as const,
      coord: Object.freeze({ q: 0, r: 0 }),
      world: Object.freeze({ x: 0, z: 0 }),
      rotation: 0,
      scale: 1
    });
    const layer = createRealmTerrainFeatureLayers(
      Object.freeze({
        points: Object.freeze([...generated.points, historicalBloom]),
        counts: Object.freeze({ ...generated.counts, 'heath-bloom': 1 }),
    ...
    const monolith = layer.group.getObjectByName(
      'realm-ancient-monoliths'
    ) as THREE.InstancedMesh | undefined;

    expect(layer.counts['heath-bloom']).toBe(0);
    expect(layer.instanceCount).toBe(generated.points.length);
    expect(layer.group.getObjectByName('realm-heath-blooms')).toBeUndefined();
    expect(monolith).toBeInstanceOf(THREE.InstancedMesh);
    expect((monolith!.material as THREE.MeshStandardMaterial).color.getHexString()).toBe('6f746c');
    layer.dispose();
  });
