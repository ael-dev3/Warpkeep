...
describe('low-poly grass geometry', () => {
  it.each(profiles)('pins %s ribbon, triangle, and planted-root contracts', (profile) => {
    ...
    const flex = geometry.getAttribute('grassFlex');
    ...
    expect(flex.count).toBe(positions.count);
    ...
    for (let ribbon = 0; ribbon < ribbons; ribbon += 1) {
      const base = ribbon * 5;
      expect(flex.getX(base)).toBe(0);
      expect(flex.getX(base + 1)).toBe(0);
      expect(flex.getX(base + 2)).toBeCloseTo(0.56, 6);
      expect(flex.getX(base + 3)).toBeCloseTo(0.56, 6);
      expect(flex.getX(base + 4)).toBe(1);
