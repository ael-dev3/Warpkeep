...
it(
  "joins actual Git closure, genuine generated releases, private corpus and bridge through the candidate",
  async () => {
    const actual =
    ...
    gitSeam.run.mockImplementation((_executable, argv, options) => {
      const output = actual.execFileSync(gitExecutable, argv, options);
...
it("validates source-rebound G001 fixture before filesystem-heavy joined composition", () => {
  expect(() =>
    sourceBoundFixture({
      commit: "d".repeat(40),
      tree: "e".repeat(40),
