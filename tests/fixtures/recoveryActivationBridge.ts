...
export function recoveryOperationAuthority(operation: string) {
...
export async function recoveryProtectedContext(
  ...
  const sourceAuthority = recoveryOperationAuthority(operation);
...
export async function recoveryActivationBridge(
  ...
  const bridge = createSealedRealmsProductionAuthBridgeState({
    authority: recoveryOperationAuthority('activation-evidence-inspect'), privateState: state,
  ...
  for (const [index, lane] of (['g002', 'ptr'] as const).entries()) {
    ...
    const inspect = await recoveryProtectedContext(state, `${lane}-import-inspect`, String(80001 + index * 2));
    await issueSealedRealmsProductionContinuation({ ...inspect, kind: `${lane}-import`, ...binding });
    const apply = await recoveryProtectedContext(state, `${lane}-import-apply`, String(80002 + index * 2));
