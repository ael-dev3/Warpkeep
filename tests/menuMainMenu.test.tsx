...
describe('WarpkeepMainMenu', () => {
  ...
  it('refreshes anchored notices and lets patch notes replace them without stealing focus', () => {
    ...
    expect(screen.queryByRole('region', { name: 'THE PETITION STANDS' })).toBeNull();

    fireEvent.click(patchNotes, { detail: 0 });
    expect(screen.queryByRole('status')).toBeNull();
    expect(screen.getByRole('region', { name: 'THE PETITION STANDS' })).not.toBeNull();
  ...
  it('reveals current patch notes in-site and lets Escape dismiss them before leaving the menu', () => {
    ...
    fireEvent.click(patchNotes, { detail: 0 });
    const notes = screen.getByRole('region', { name: 'THE PETITION STANDS' });
    expect(patchNotes.getAttribute('aria-expanded')).toBe('true');
    expect(notes.textContent).toContain('LATEST PATCH · ALPHA 0.3.34');
    expect(notes.textContent).toContain('disabled Request Sent action');
    expect(notes.textContent).toContain('already recorded by SpacetimeDB');
    expect(notes.textContent).toContain('repeated gestures are absorbed locally');
  ...
  it('keeps hover notes reachable across the anchor gap and toggles by activation', () => {
    ...
    const panel = screen.getByRole('region', { name: 'THE PETITION STANDS' });
