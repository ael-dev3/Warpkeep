...
function getPatchNotesTrigger(options: { hidden?: boolean } = {}) {
  return screen.getByRole('button', {
    ...options,
    name: 'Open patch notes for Warpkeep ALPHA 0.3.39'
  });
}
...
describe('WarpkeepMainMenu', () => {
  ...
  it('refreshes anchored notices and lets patch notes replace them without stealing focus', () => {
    ...
    expect(screen.queryByRole('region', { name: 'THE PETITION HOLDS' })).toBeNull();

    fireEvent.click(patchNotes, { detail: 0 });
    expect(screen.queryByRole('status')).toBeNull();
    expect(screen.getByRole('region', { name: 'THE PETITION HOLDS' })).not.toBeNull();
  ...
  it('reveals current patch notes in-site and lets Escape dismiss them before leaving the menu', () => {
    ...
    fireEvent.click(patchNotes, { detail: 0 });
    const notes = screen.getByRole('region', { name: 'THE PETITION HOLDS' });
    expect(patchNotes.getAttribute('aria-expanded')).toBe('true');
    expect(notes.textContent).toContain('LATEST PATCH · ALPHA 0.3.39');
    expect(notes.textContent).toContain('Request Sent');
    expect(notes.textContent).toContain('violet-and-gold seal');
    expect(notes.textContent).toContain('one private petition');
  ...
  it('keeps hover notes reachable across the anchor gap and toggles by activation', () => {
    ...
    const panel = screen.getByRole('region', { name: 'THE PETITION HOLDS' });
