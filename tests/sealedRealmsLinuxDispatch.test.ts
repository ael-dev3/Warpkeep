...
async function fixture(operation: string, failure?: string) {
 vi.stubEnv('WARPKEEP_OPERATION',operation);vi.stubEnv('GITHUB_JOB',failure==='job'?'wrong-job':operation.endsWith('generate')?'operate':'operate_readonly');
 const calls: string[]=[];
 const factory='createSealedRealmsProductionActivationWorkflowRuntime';
 const run='runSealedRealmsProductionActivationOperation';
 ...
 const state={calls,selected:{path:'scripts/fixed.bundle.mjs',factoryExport:factory,exportNames:[factory,run]},loaded:{
  ...
  [run]:async(input: {runtime: unknown})=>{calls.push('run');expect(input.runtime).toBe(runtime);return {operation,status:failure==='result'?'unexpected':operation.endsWith('inspect')?'activation-evidence-inspected':'completed'};}
 }};
 ...
 substitute('function bundle(commit, lane)', '/** Fixed Linux operating caller',`function bundle(commit,lane){if(lane!=='activation')throw Error('wrong lane');globalThis.${stateKey}.calls.push('bundle');return globalThis.${stateKey}.selected;}`);
 ...
 finally {vi.unstubAllEnvs();delete (globalThis as Record<string,unknown>)[stateKey];}
}
it.each(['activation-evidence-inspect','activation-evidence-generate'])('dispatches %s through fixed factory, opaque runtime and reattestation',async operation=>{
 const result=await fixture(operation);expect(result.result).toEqual({operation,status:operation.endsWith('inspect')?'activation-evidence-inspected':'completed'});
