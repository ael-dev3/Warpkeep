...
describe('graphics preference', () => {
  ...
  it('defaults every capability shape to cinematic until a player deliberately opts down', () => {
    [
      [1_440, 900],
      [320, 568],
      [360, 800],
      ...
      expect(resolveGraphicsQuality(DEFAULT_GRAPHICS_PREFERENCE, {
        ...
        height,
        devicePixelRatio: 3,
        hardwareConcurrency: width <= 430 ? 2 : 8,
        deviceMemory: width <= 430 ? 2 : 8,
        maxTextureSize: width <= 430 ? 2_048 : 16_384
      })).toBe('cinematic');
    });
  });
