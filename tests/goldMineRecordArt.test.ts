...
const MANIFEST_PATH = resolve(
  ROOT,
  'docs/reference/resources/2026-07-18-hegemony-gold-mine/record-art/manifest.json'
);
...
describe('Hegemony Gold Mine record art', () => {
  it('pins the narrow visual authorization without promoting a Gold Mine into gameplay', () => {
    expect(manifest.schemaVersion).toBe(1);
    expect(manifest.projectAuthorization.scope).toContain('PR #49');
  ...
  it('keeps the asset local and decorative in the validated Realm inspection panel', () => {
    ...
    expect(component).toContain("publicAssetUrl('images/realm/hegemony-gold-mine-record.webp')");
    expect(component).toContain('className="gold-mine-inspection__hero-art"');
