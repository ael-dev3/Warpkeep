...
it('shows captured Worker rates and allows recall only before returning with atlas authority', () => {
  ...
  const reserved = screen.getByText('Reserved result: 60 food · pending until the Realm confirms return.');
  expect(reserved).toBeVisible(); expect(reserved).toHaveAttribute('role', 'status');
