...
it.skipIf(process.platform !== 'linux')('runs actual checked-in and activation Verify CLI on native V2/V4 Git history', () => {
  ...
  try {
    ...
    const adoptionBinding = createRecoveryActivationBindingFromCandidate(canonical(Object.fromEntries(
      recoveryBindingKeys(4).map(key => [key, adoptionValues[key]]))));
    ...
    for (const phase of ['checked-in', 'activation']) {
      ...
      expect(summary).toMatchObject({ schemaVersion: 4,
        profile: 'warpkeep-0.4.0-sealed-launch-ptr-adoption-v4', phase: 'activation', pagesDeploymentApproved: true });
