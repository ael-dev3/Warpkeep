      ...
      args => ({
        ...
        contents: args.path === 'castle-worker-authority'
          ? `
              ...
              export function projectMyWorkerState(ctx, fid, now) {
  ...
  const ctx = {
    ...
    db: {
      ...
      castleInnerKeepBuildingV1: {
        ...
        slotKey: {
          find: (key: string) => [...buildings.values()].find(row => row.slotKey === key) ?? null,
        },
        buildingKey: {
        ...
        insert: (row: AnyRow) => {
          ...
          if ([...buildings.values()].some(existing => existing.slotKey === row.slotKey)) {
            throw new Error('TEST_SLOT_DUPLICATE');
          }
          buildings.set(row.buildingKey, { ...row });
          return row;
        },
...
test('a serialized twenty-request distinct-key burst accepts one project exactly once', () => {
  ...
  assert.throws(
    () => fixture.start('lumber-camp', requestKey('tab-b'), 'inner-keep-slot-m02'),
    /INNER_KEEP_BUILDER_BUSY/,
  );
  for (let index = 0; index < 18; index += 1) {
    assert.throws(
      () => fixture.start('city-stoneworks', requestKey('rapid', index), 'inner-keep-slot-m03'),
      /INNER_KEEP_BUILDER_BUSY/,
    );
  }
