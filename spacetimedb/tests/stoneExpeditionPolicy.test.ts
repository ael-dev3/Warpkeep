...
import { genesisWaterRiverCellSet } from '../src/waterWorld';
import {
...
test('the Tier-I Stone catalog is pinned to 96 broadly distributed heath sites', () => {
  ...
  const woodKeys = new Set(CANONICAL_TIER_I_WOOD_SITES_V1.map(site => `${site.q},${site.r}`));
  const riverKeys = genesisWaterRiverCellSet();
  ...
  const eligibleCandidates = CANONICAL_WORLD_TILES.filter(tile => {
    ...
    return meta?.passable === true
      ...
      && !woodKeys.has(tile.key)
      && !riverKeys.has(tile.key)
  ...
  assert.equal(GENESIS_TIER_I_STONE_SITE_CANDIDATE_COUNT, 159);
  ...
  for (const site of CANONICAL_TIER_I_STONE_SITES_V1) {
    ...
    assert.equal(woodKeys.has(key), false);
  ...
  assert.equal(STONE_SITE_POLICY_VERSION, 'genesis-001-tier1-stone-sites-v3');
});
