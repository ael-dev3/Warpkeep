...
test('the migration verifier retains the populated v10 to v11 proof inside protocol v13', () => {
  ...
  assert.match(verifier, /v13 boundary must refuse its immediate predecessor/);
  assert.match(verifier, /deployedV11Tables/);
  assert.match(verifier, /populated Water\/Stone\/Water-revision fixtures remained preserved through v13/);
  ...
  assert.deepEqual(
    ...
    new Set(['additiveV13SchemaFixture']),
  );
});
