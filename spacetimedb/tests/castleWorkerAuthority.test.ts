...
test('recall caps server-time accrual, deletes stale schedules, and stores no no-op receipt', () => {
  const authority = source('../src/castleWorkerAuthority.ts');
  ...
  assert.match(recall, /if \(assignment === null\)[\s\S]*return;/);
  assert.match(recall, /if \(assignment\.phase === 'returning'\) return;/);
  assert.match(recallAll, /if \(lastAssignmentId === undefined\) return;/);
...
test('founding updates active roster readiness atomically and legacy dispatch is retired', () => {
  ...
  assert.match(roster, /realmWorkerSystemV1\.realmId\.update\(\{[\s\S]*expectedCastleCount: castleIds\.length,[\s\S]*expectedWorkerCount: castleIds\.length \* CASTLE_WORKERS_PER_CASTLE,[\s\S]*rosterDigest: rosterDigestForCastleIds\(castleIds\)/);
...
test('worker assignments and replay receipts remain bound to canonical castle ownership', () => {
  ...
  for (const replay of [dispatch, recall, recallAll]) {
    assert.match(replay, /workerReceiptShapeIsValid\(prior\)/);
