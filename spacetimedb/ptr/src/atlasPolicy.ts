...
export const GREATER_REALM_RENDERER_CONTRACT_VERSION =
  'greater-realm-renderer-v1';
export const GREATER_REALM_PUBLIC_RELEASE_ID_PATTERN = /^GRR-[A-Z2-7]{26}$/u;
export const GREATER_REALM_CHUNK_HANDLE_PATTERN = /^GRK-[A-Z2-7]{26}$/u;
export const GREATER_REALM_RESOURCE_LOCATION_ID_PATTERN = /^GRL-[A-Z2-7]{26}$/u;
...
export const GREATER_REALM_CHUNK_BIN_SIZE = 15;
...
function requireGreaterRealmPatternId(
  value: string,
  pattern: RegExp,
  code: string,
): string {
...
export function requireGreaterRealmChunkHandle(value: string): string {
  return requireGreaterRealmPatternId(
    value,
    GREATER_REALM_CHUNK_HANDLE_PATTERN,
    'GREATER_REALM_CHUNK_HANDLE_INVALID',
  );
}
...
export function validateGreaterRealmReleaseInputV1(input: GreaterRealmReleaseInputV1): void {
  ...
  requireGreaterRealmPatternId(
    input.publicReleaseId,
    GREATER_REALM_PUBLIC_RELEASE_ID_PATTERN,
    'GREATER_REALM_PUBLIC_RELEASE_ID_INVALID',
  );
  ...
  if (input.rendererContractVersion !== GREATER_REALM_RENDERER_CONTRACT_VERSION) {
    fail('GREATER_REALM_RENDERER_VERSION_INVALID');
  }
...
export function validateGreaterRealmChunkInputV1(input: GreaterRealmChunkInputV1): void {
  requireGreaterRealmChunkHandle(input.chunkHandle);
  requireGreaterRealmOpaqueId(input.atlasId, 'GREATER_REALM_ATLAS_ID_INVALID');
  ...
  if (input.chunkCoordKey !== `B:${input.binQ}:${input.binR}`) {
    fail('GREATER_REALM_CHUNK_COORD_KEY_INVALID');
  }
...
export function validateGreaterRealmCellInputV1(input: GreaterRealmCellInputV1): void {
  ...
  if (input.passable && input.movementCost >= 1_000_000) {
    fail('GREATER_REALM_MOVEMENT_COST_INVALID');
  }
  ...
  if (input.wildflowerBasisPoints > input.groundcoverBasisPoints) {
    fail('GREATER_REALM_DENSITY_INVALID');
  }
}
...
export function validateGreaterRealmResourceInputV1(input: GreaterRealmResourceInputV1): void {
  ...
  requireGreaterRealmPatternId(
    input.locationId,
    GREATER_REALM_RESOURCE_LOCATION_ID_PATTERN,
    'GREATER_REALM_RESOURCE_LOCATION_INVALID',
  );
  requireGreaterRealmOpaqueId(input.cellKey, 'GREATER_REALM_CELL_KEY_INVALID');
  requireGreaterRealmPublicRegion(input.regionId);
  requireGreaterRealmOpaqueId(input.componentKey, 'GREATER_REALM_COMPONENT_KEY_INVALID');
