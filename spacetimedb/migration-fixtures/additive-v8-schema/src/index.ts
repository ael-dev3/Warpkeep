...
// Protocol-v8 is an append-only suffix after the independent Food expedition
// tables. These schema-only definitions deliberately mirror production's
// public catalog/occupation/schedule boundary and private expedition state.
const woodSiteV1 = table({ name: 'wood_site_v1', public: true }, {
  siteId: t.string().primaryKey(), q: t.i32(), r: t.i32(), tier: t.u32(), active: t.bool(),
});
const woodNodeOccupationV1 = table({
  name: 'wood_node_occupation_v1', public: true,
  indexes: [{ accessor: 'byOriginCastle', algorithm: 'btree', columns: ['originCastleId'] as const }] as const,
}, {
  siteId: t.string().primaryKey(), originCastleId: t.u64(), phase: t.string(), startedAtMicros: t.u64(),
  arrivesAtMicros: t.u64(), gatheringEndsAtMicros: t.u64(), returnsAtMicros: t.u64(),
});
const woodExpeditionV1 = table({
  ...
  siteId: t.string().index(), phase: t.string(), startedAtMicros: t.u64(), arrivesAtMicros: t.u64(),
  gatheringEndsAtMicros: t.u64(), returnsAtMicros: t.u64(), settledThroughMicros: t.u64(),
  accruedWood: t.u64(), creditedWood: t.u64(), policyVersion: t.string(), createdAt: t.timestamp(), updatedAt: t.timestamp(),
});
const woodExpeditionIdempotencyV1 = table({ name: 'wood_expedition_idempotency_v1' }, {
  requestKey: t.string().primaryKey(), fid: t.u64().index(), siteId: t.string(),
  expeditionId: t.string().unique(), createdAt: t.timestamp(),
});
const woodExpeditionScheduleV1 = table({
  name: 'wood_expedition_schedule_v_1', public: true,
...
/**
 * Disposable loopback-only predecessor fixture. It retains every v5-through-v8
 * append so inspection never requests a destructive schema downgrade.
 */
export const fixtureSeedGenesisGenerationV2 = db.reducer(
  ...
  ctx => {
    if (
      ...
      || ctx.db.woodSiteV1.count() !== 0n
      || ctx.db.woodNodeOccupationV1.count() !== 0n
      || ctx.db.woodExpeditionV1.count() !== 0n
      || ctx.db.woodExpeditionIdempotencyV1.count() !== 0n
