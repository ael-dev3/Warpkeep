...
function assertLegacySlotMapping(ctx: GreaterRealmContext, row: GreaterRealmSlotInputV1): void {
  ...
  const canonical = canonicalCastleSlotForId(row.legacySlotId);
  const cell = ctx.db.greaterRealmCellV1.cellKey.find(row.cellKey);
  if (
    canonical === undefined
    || cell === null
    || cell.localQ !== canonical.q
    || cell.localR !== canonical.r
...
export function finalizeGreaterRealmReleaseV1(
  ...
  random: Pick<Random, 'integerInRange'>,
): void {
  ...
  if (slots.length !== GREATER_REALM_CASTLE_CAPACITY) fail('GREATER_REALM_SLOT_COUNT_INVALID');
  const shuffled = fisherYatesGreaterRealmRanks(
    slots,
    (minimum, maximum) => random.integerInRange(minimum, maximum),
  );
  const regionRanks = new Map<string, number>();
  shuffled.forEach((slot, allocationRank) => {
    const regionOrderRank = regionRanks.get(slot.regionId) ?? 0;
    regionRanks.set(slot.regionId, regionOrderRank + 1);
    ctx.db.greaterRealmCastleSlotV1.slotId.update({
      ...slot,
      regionOrderRank,
      allocationRank,
    });
  ...
  for (const region of GREATER_REALM_PUBLIC_REGIONS) {
    if (regionRanks.get(region.id) !== GREATER_REALM_CASTLES_PER_REGION) {
      fail('GREATER_REALM_REGION_SLOT_RANK_INVALID');
    }
  }
  ctx.db.greaterRealmReleaseV1.atlasId.update({
    ...release,
