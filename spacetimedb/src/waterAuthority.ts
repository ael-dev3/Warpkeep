...
export function assertGenesisWaterWorldPrecondition(snapshot: GenesisSeedSnapshot): void {
  if (classifyGenesisStaticSnapshot(snapshot) !== 'generation-v3') {
...
function inspectLayout(ctx: WarpkeepReducerContext): WaterLayoutRow | undefined {
  ...
  for (const row of ctx.db.realmWaterLayoutV1.iter()) {
    ...
    if (
      found !== undefined
      || !matchesGenesisWaterLayoutV1(row)
