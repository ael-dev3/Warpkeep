...
The exact `b306eed` source completed full native preparation and an independent
rebuilding check. Generated-only commit `ba12a7f` integrates the
verified output delta. Clean candidate QA passed the complete affected release and
recovery-policy suites and both type checks. Later panel-closure fix
`0d98599` passed its affected navigation/UI tests and types; its source
manifest is derived separately. These later source/documentation changes do not
extend the full-preparation proof beyond its actual `b306eed` input. Check the
actual branch/CI state before operating; final release preparation remains open.
