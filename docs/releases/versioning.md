...
| --- | --- | --- |
| Checked-in product candidate | Player-facing semantic version in source | `ALPHA 0.3.4` (not deployed) |
...
The checked-in package identifies the next Alpha 0.3.4 candidate. The verified
public release remains Alpha 0.3.3 until the exact candidate commit passes the
full release matrix, is published through protected main, and passes exact-build
post-deploy checks. Product version, authentication contract, backend protocol,
realm seed, and build SHA are independent coordinates; changing one does not
