...
The checked-in Alpha 0.3.11 candidate is deliberately ahead of that live
...
wagon loop, the shared forest suffix, a bounded 96-site Tier-I Wheat Farm Food
loop, and a bounded 96-site Tier-I Logging Camp Wood loop. It does not attest a
production publish, world transition, resource backfill, Gold/forest/Food/Wood
setup, or deployment. Alpha 0.3.6 remains the verified public release.
...
| Checked-out economy candidate | resource authority v4, Gold expedition tables v5, forest tables v6, Food expedition tables v7, and Wood expedition tables v8 (all undeployed) |
...
additive-v6, additive-v7, and additive-v8 fixtures plus the checked-out module
with `--delete-data=never`; proves refs 0–18 and their rows remain exact while
private `resource_account_v1` appends at ref 19, Gold at refs 20–24, forest at
refs 25–26, Food at refs 27–31, and Wood at refs 32–36. It verifies empty and
synthetic non-empty fixture preservation, idempotent artifact republish, the
existing resource-account lifecycle, Gold/Food/Wood table order and
public/private shapes, populated 1,261-to-10,000 expansion, target retry,
partial state, and rollback refusal. It does not seed, dispatch, or settle a
Food or Wood expedition. Focused authority, policy, and reducer-contract tests
separately cover paired Food/Wood reservations across reads, collection, expiry,
and concurrent Gold settlement. The migration proof never contacts Maincloud and
is neither production inspection nor production publish, expansion, or
Gold/forest/Food/Wood setup approval.
...
| `wood_site_v1` | public | Candidate immutable catalog of exactly 96 canonical Tier-I Logging Camp coordinates, tier, and active state. |
| `wood_node_occupation_v1` | public | Candidate identity-minimized Logging Camp occupancy: site, public origin castle, phase, and server-derived lifecycle timestamps. |
| `wood_expedition_v1` | private | Candidate FID-bound Wood-wagon lifecycle, route-derived timing, private settlement cursor, and accrued/credited Wood. |
| `wood_expedition_idempotency_v1` | private | Candidate caller-request receipt that makes same-request Wood dispatch bounded and exactly-once. |
| `wood_expedition_schedule_v_1` | public-safe scheduler projection | Candidate minimal Wood schedule target; it has only already-public lifecycle fields, and its target reducer is internal-only rather than a gameplay subscription surface. |
...
tables at refs 20–24, the two forest tables at refs 25–26, and the five Food
tables at refs 27–31. The Alpha 0.3.11 candidate then appends the five Wood
tables at refs 32–36. Generated browser bindings omit accessors for every
private resource/Gold/Food/Wood authority table. The active browser keeps its
six inherited Realm projections—`world_tile`, `world_tile_meta_v1`, `player_v2`,
`castle`, `realm_v1`, and `realm_profile_v1`—and may additionally subscribe
only to the public Gold, Food, and Wood site/occupation projections plus the
validated public forest layout/instances. It does not subscribe to the frozen
legacy `player`, the slot-plan table, or any public-safe scheduler projection.
...
separate compiled Food policy selects exactly 96 passable Lowland/Meadow
resource-capable anchors and pins the resulting fixed catalog by policy version
and digest. The Alpha 0.3.11 Wood policy separately selects exactly 96 passable
Forest resource-capable anchors, excludes the Gold/Food catalogs, forest
clearance, castle clearance, and protected-corridor clearance, and pins its own
catalog by version and digest. Labels and GLB bytes never create browser
authority, a balance, a resource yield, a building, unit, combat, alliance, chat,
or season state.
...
The Alpha 0.3.11 Wood candidate likewise does not treat a v4 resource aggregate,
a Gold/Food aggregate, or a forest-layout check as a v8 attestation. Any future
Wood installation needs its own reviewed, identity-safe aggregate contract for
refs 32–36, the exact 96-site policy/version/digest, public occupation, private
idempotency/expedition invariants, and the paired Food/Wood reservation
invariant. No aggregate may expose FIDs, routes, request keys, balances, or
accrued output.
...
The same separation applies to the Alpha 0.3.11 Wood v8 suffix. A Logging Camp
asset delivery, local migration proof, Gold/Food/forest setup, or v4 resource
checkpoint does not authorize publication of refs 32–36, Wood-site seeding, a
worker change, Pages deployment, custom-domain/DNS modification, or enablement.
Each requires its own explicit owner approval after a fresh read-only
precondition.
...
coordinates, the checked-in Alpha 0.3.11 candidate carries the bounded private
resource loop, 10,000-cell world definition, a 24-site Gold Mine wagon pilot,
a 96-site Tier-I Wheat Farm loop, and a 96-site Tier-I Logging Camp loop. Gold,
Food, and Wood may each run one wagon from the same castle concurrently, but all
ownership, capacity, route, time, reward, and replay decisions remain
server-side. Food and Wood each credit exactly one completed server minute for
at most 30 days. Raw passive Food and Wood through each deadline remain paired
private reservations, so delayed schedules, cross-resource collection, and
concurrent Gold expiry cannot truncate or duplicate either award. These are
undeployed candidate authorities, not a public release or a production state
claim. No other resource nodes, queues, units, scouting, combat, alliances,
seasons, or activity reports are introduced. AI may produce flavor or summaries,
