...
fixture paths were absent from the real scanner regression's fixture inventory.
The correction adds both to exact public-thumbprint, mutated-value and wrong-path
checks without changing `.gitleaks.toml`. RED `51535` reproduced two intended
failures; GREEN `92981` passed all seven owning tests. The installed pinned
Gitleaks 8.30.1 accepted all 42 allowed fixtures and detected all 82 required
negative findings. Root diff review and file-size checks passed. Product and
workflow integration reviews found no blocking defect. OIDC authenticates the
shared PTR job; the fixed caller, opaque permit and continuation separately prove
apply authority. Draft PR #244 now targets main, retaining all reviewed stack
changes; its next push requires fresh hosted acceptance for that consolidated base.

Integration `70a65b34d7c814657cba1381733a73712736c70b` is published and was
synchronized to the clean native checkout while idle. Native consumer run `35676`
is closed with exit zero: all 701 tests across 18 owning generated/private suites
passed without skips in 339.98 seconds. This includes the previously failing
source-pin CLI, retained adoption, activation/bridge, descriptor generation and
prepared-family consumers. Native preflight `7574` also closed with exit zero:
all 52 cases passed without skips in 21.96 seconds. The positive traversed the
real activation closure; graph negatives proved that closure was never reached.
Two earlier preflight attempts stopped during root-to-owner Git clone setup,
before native cases ran. A parent-held memory file supplied only the exact donor
Git directory as safe for that test process; it was closed afterward and never
entered the production child environment. No persistent Git configuration changed,
and the fixture removed its own temporary roots. Native remained clean and idle.
Both native sealed verifiers passed in preparation state; Pages routing returned
`sealed-launch-blocked`. These results close the native limits below, not live
deployment acceptance. The later scanner-only coverage correction does not change
runtime or generated bytes; its own verification is recorded with that change.
...
file-size policy and staged diff checks passed. Native preflight and the generated
consumers subsequently passed against the committed integration as recorded above;
the uncommitted candidate cannot serve as their Git donor. This is development
