...
function routePresentation(
  ...
  if (cameraMode === 'realm') return undefined;
...
function routeIsEligible(
  ...
  if (worker.workerId === selectedWorkerId) return true;
  if (worker.status === 'gathering') {
    return cameraMode !== 'realm' && worker.workerId === hoveredWorkerId;
  }
  return worker.ownedByViewer
    || (cameraMode !== 'realm' && worker.workerId === hoveredWorkerId);
}
...
export function createRealmWorkerRouteLayer(
  ...
  // Standalone layers preserve the historical strategy presentation until
  // the owning scene supplies its current camera band.
  let cameraMode: RealmCameraMode = 'approach';
