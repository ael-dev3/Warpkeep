...
import {
  ...
  type RealmStoneWagonPose
} from './realmStoneNodePresentation';

const HEX_SIZE = 1;
const RESOURCE_GROUND_LIFT = 0.018;
const STONE_QUARRY_TARGET_FOOTPRINT = 1.22;
const WAGON_TARGET_FOOTPRINT = 0.64;

export type RealmStoneNodeSceneRecord = RealmStoneNodePresentation;

export type RealmStoneNodeInstanceHit = Readonly<{
  siteId: string;
  coord: HexCoord;
  source: 'site' | 'wagon';
  distance: number;
...
export type RealmStoneNodePresentationTelemetry = Readonly<{
  publicSiteCount: number;
  occupiedSiteCount: number;
  renderedStoneQuarryCount: number;
...
export type RealmStoneNodeLayer = Readonly<{
  ...
  canReconcile: (sites: readonly RealmStoneNodeSceneRecord[]) => boolean;
  ...
  getPresentationTelemetry: () => RealmStoneNodePresentationTelemetry;
  dispose: () => void;
}>;

export type CreateRealmStoneNodeLayerOptions = Readonly<{
  sites: readonly RealmStoneNodeSceneRecord[];
  ...
  /** Scene-owned Stone slice from the shared Gold/Food/Wood/Stone scene allocator. */
...
function chooseQuarryLod(quality: RealmQualitySpec, distance: number): HegemonyExpeditionLod | undefined {
...
/**
 * A Stone-only visual layer. Missing Quarry GLBs leave public fallback glyphs in
 * place; malformed Stone input has already been reduced to zero sites by the
 * Stone presentation policy and can never disturb Gold or core terrain.
 */
export function createRealmStoneNodeLayer(options: CreateRealmStoneNodeLayerOptions): RealmStoneNodeLayer {
  const group = new THREE.Group();
  group.name = 'realm-stone-node-layer';
  ...
  fallbackGroup.name = 'realm-stone-node-fallback-markers';
  const modelGroup = new THREE.Group();
  modelGroup.name = 'realm-stone-node-models';
  ...
  markers.name = 'realm-stone-node-markers';
  ...
  const occupiedMaterial = new THREE.MeshBasicMaterial({
    color: '#d8b9ef', transparent: true, opacity: 0.86, depthWrite: false,
  ...
  selectedRing.name = 'realm-stone-node-selection-ring';
  ...
  pickVolumes.name = 'realm-stone-node-pick-volumes';
  ...
  const maximumRenderedQuarries = budget?.maximumRenderedNodes
    ?? HEGEMONY_STONE_QUARRY_RENDER_LIMITS.maximumRenderedNodes[options.quality.id];
  ...
  wagonPickVolumes.name = 'realm-stone-wagon-pick-volumes';
  ...
  const wagonFallbackMaterial = new THREE.MeshBasicMaterial({
    color: '#aeb5b2', transparent: true, opacity: 0.9, depthWrite: false, toneMapped: false
  });
  ...
  wagonFallbackMarkers.name = 'realm-stone-wagon-fallback-markers';
  ...
  const syncFallbackMarkers = () => {
    const presentedSiteIds = [...visualInstances.values()]
      .filter((visual) => visual.kind === 'stone-quarry')
      .map((visual) => visual.key.slice('quarry:'.length))
      .sort();
  ...
  const replaceVisuals = (camera: THREE.PerspectiveCamera, nowMicros: bigint, elapsedSeconds: number) => {
    ...
    for (const candidate of quarryCandidates) {
      const requested = chooseQuarryLod(options.quality, candidate.distance);
      const lod = requested ? loadedLodFor('stone-quarry', requested) : undefined;
      if (lod) desired.set(`quarry:${candidate.node.record.siteId}`, Object.freeze({
        kind: 'stone-quarry', lod, node: candidate.node, animate: false
      }));
    }
    const wagonCandidates = nodes
      .map((node) => {
        const pose = resolveRealmStoneWagonPose(node.record, nowMicros);
  ...
  const loadModels = async () => {
    ...
    for (const lod of qualityLods(options.quality)) {
      tasks.push(acquireHegemonyExpeditionPrefab({
        label: `Hegemony Stone Quarry ${lod}`,
        asset: HEGEMONY_STONE_QUARRY_RUNTIME_ASSETS[lod],
        materialRole: 'gathering-node',
        baseUrl: options.baseUrl,
          targetFootprintDiameter: STONE_QUARRY_TARGET_FOOTPRINT,
        dynamicShadows: options.quality.dynamicShadows,
        maxAnisotropy: options.maxAnisotropy,
        signal: modelAbortController.signal
      }).then((model) => {
        const key = modelMapKey('stone-quarry', lod);
  ...
  const canReconcile = (sites: readonly RealmStoneNodeSceneRecord[]) => {
  ...
  const update = (camera: THREE.PerspectiveCamera, nowMicros: bigint, elapsedSeconds: number) => {
    ...
    const after = `${telemetry.renderedStoneQuarryCount}:${telemetry.renderedWagonCount}:${telemetry.animatedWagonCount}`;
...
export const HEGEMONY_STONE_QUARRY_ASSET_BUDGETS = Object.freeze({
  stoneQuarryTargetFootprint: STONE_QUARRY_TARGET_FOOTPRINT,
