...
export type RealmWoodNodeLayer = Readonly<{
  ...
  canReconcile: (sites: readonly RealmWoodNodeSceneRecord[]) => boolean;
...
export type CreateRealmWoodNodeLayerOptions = Readonly<{
  ...
  /** Scene-owned Wood slice from the shared Gold/Food/Wood/Stone scene allocator. */
...
export function createRealmWoodNodeLayer(options: CreateRealmWoodNodeLayerOptions): RealmWoodNodeLayer {
  ...
  wagonPickVolumes.name = 'realm-wood-wagon-pick-volumes';
  ...
  const wagonFallbackMaterial = new THREE.MeshBasicMaterial({
    color: '#b4864f', transparent: true, opacity: 0.9, depthWrite: false, toneMapped: false
  });
  ...
  wagonFallbackMarkers.name = 'realm-wood-wagon-fallback-markers';
  ...
  const canReconcile = (sites: readonly RealmWoodNodeSceneRecord[]) => {
  ...
  const raycast = (raycaster: THREE.Raycaster) => {
    ...
    const siteCandidate: RealmWoodNodeInstanceHit | null = siteNode && siteHit
      ? Object.freeze({
      ...
      : null;
    const wagonCandidate: RealmWoodNodeInstanceHit | null = wagonNode && wagonHit
      ? Object.freeze({
