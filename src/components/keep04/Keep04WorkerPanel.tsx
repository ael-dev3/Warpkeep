...
export function Keep04WorkerPanel({ view, enabled, nowMs, onRecall, onFindResources }: Readonly<{
  ...
  return <section aria-label="Workers" className="keep04-worker-panel">
    <h2>Workers</h2>
    <p>Choose an actual resource location in the Greater Realm to dispatch an idle Worker.</p>
    {view.workers.map(worker => <article key={worker.ordinal} aria-label={`Worker ${worker.ordinal + 1}`} className="keep04-card">
      ...
      {worker.capturedYield !== null && <p>Captured yield: {worker.capturedYield.toString()} per 10-second quantum</p>}
    ...
    <p>Existing expeditions keep their captured rates. Resources become spendable only after the Realm confirms their return.</p>
  </section>;
}
