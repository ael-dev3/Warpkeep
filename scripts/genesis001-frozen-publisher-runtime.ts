...
const RECOVERY_PROFILE = 'warpkeep-genesis-001-frozen-publish-recovery-v1';
...
function assertTrustedReadableDirectory(path: string, label: string): void {
  ...
  if (
    ...
    || (status.mode & 0o022) !== 0
    || (owner() !== undefined && status.uid !== owner())
  ) fail(`${label} is not a trusted read-only dependency directory`);
}
...
function executableAttestation(path: string): Readonly<{ verify: () => void }> {
  if (!isAbsolute(path) || resolve(path) !== path || realpathSync.native(path) !== path) {
    fail('Genesis 001 dependency executable path must be exact and absolute');
  }
  const before = lstatSync(path, { bigint: true });
  if (
    !before.isFile()
    ...
    || (owner() !== undefined && before.uid !== BigInt(owner()!))
  ) fail('Genesis 001 dependency executable is unsafe');
  ...
  return Object.freeze({
    verify: () => {
      ...
      ) fail('Genesis 001 dependency executable changed after attestation');
    },
  });
}
...
export function ensureGenesis001FrozenWorkspaceRoot(explicit?: string): string {
  ...
  const root = explicit ?? join(parent, 'genesis001-frozen-publish-v1');
  if (!existsSync(root)) createPrivateDirectory(root);
  assertPrivateDirectory(root, 'Genesis 001 workspace root');
  return root;
}

function buildEnvironment(runRoot: string): Readonly<Record<string, string>> {
...
function installAndBuildHistoricalModule(input: Readonly<{
  moduleRoot: string;
  runRoot: string;
  environment: Readonly<Record<string, string>>;
  pnpmExecutablePath: string;
...
export async function buildImmutableGenesis001FrozenArtifact(input: Readonly<{
  ...
  try {
    const environment = buildEnvironment(runRoot);
    const pnpm = executableAttestation(configuration.pnpmExecutablePath);
    assertTrustedReadableDirectory(configuration.pnpmStorePath, 'Genesis 001 pnpm store');
    ...
    const baselineArtifactPath = installAndBuildHistoricalModule({
      moduleRoot: baselineModuleRoot,
      runRoot,
      environment,
      pnpmExecutablePath: configuration.pnpmExecutablePath,
      pnpmStorePath: configuration.pnpmStorePath,
    ...
    const artifactPath = installAndBuildHistoricalModule({
      moduleRoot,
      runRoot,
      environment,
      pnpmExecutablePath: configuration.pnpmExecutablePath,
...
export function readGenesis001LiveSnapshot(input: Readonly<{
  ...
  const descriptor = parseCanonicalSchemaDescription(runBounded({
...
export function attestGenesis001PinnedCli(
  ...
  return attestPinnedSpacetimeCli(executable, spawnSync, environment) as Genesis001PinnedCliSnapshot;
}
...
export function genesis001RuntimeConfiguration(input: Readonly<{
  ...
  for (const [label, path] of [
    ['pnpm executable', input.pnpmExecutablePath],
    ['pnpm store', input.pnpmStorePath],
  ...
  const childEnvironment = sanitizeGenesis001ChildEnvironment(input.environment);
  return Object.freeze({
    repositoryRoot,
    workspaceRoot,
    pnpmExecutablePath: input.pnpmExecutablePath,
    pnpmStorePath: input.pnpmStorePath,
