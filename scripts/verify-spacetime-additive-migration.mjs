...
const additiveV15SchemaFixture = resolve(
  repositoryRoot,
  'spacetimedb/migration-fixtures/additive-v15-schema',
);
const additiveV16SchemaFixture = resolve(
  repositoryRoot,
  'spacetimedb/migration-fixtures/additive-v16-schema',
);
const additiveV17SchemaFixture = resolve(
  repositoryRoot,
  'spacetimedb/migration-fixtures/additive-v17-schema',
);
const currentCandidateInspectionFixture = resolve(
  repositoryRoot,
...
const realmChatMigrationDatabase = 'warpkeep-migration-realm-chat-v16';
const greaterRealmMigrationDatabase = 'warpkeep-migration-greater-realm-v17';
...
const additiveV15Tables = Object.freeze([
...
const additiveV16Tables = Object.freeze([
...
const additiveV17Tables = Object.freeze([
...
const productionPlayerCanaryTables = Object.freeze([
...
const deployedV15Tables = Object.freeze([
  ...deployedV14Tables,
  ...additiveV15Tables,
]);
const deployedV16Tables = Object.freeze([
  ...deployedV15Tables,
  ...additiveV16Tables,
]);
const deployedV17Tables = Object.freeze([
  ...deployedV16Tables,
  ...additiveV17Tables,
]);
const deployedCurrentCandidateTables = Object.freeze([
  ...deployedV17Tables,
  ...productionPlayerCanaryTables,
]);
const expectedProductTypeRefs = Object.freeze({
  ...
  inner_keep_layout_v1: 56,
  inner_keep_slot_v1: 57,
  inner_keep_building_catalog_v1: 58,
  inner_keep_build_level_v1: 59,
  castle_inner_keep_building_v1: 60,
  castle_inner_builder_v1: 61,
  castle_inner_build_receipt_v1: 62,
  castle_inner_construction_schedule_v_1: 63,
  realm_chat_status_v1: 64,
  realm_chat_channel_v1: 65,
  realm_chat_message_v1: 66,
  realm_chat_recent_v1: 67,
  realm_chat_rate_event_v1: 68,
  realm_chat_send_receipt_v1: 69,
  realm_chat_report_v1: 70,
  realm_chat_report_rate_event_v1: 71,
  greater_realm_release_v1: 72,
  greater_realm_chunk_v1: 73,
  greater_realm_navigation_component_v1: 74,
  greater_realm_cell_v1: 75,
  greater_realm_castle_slot_v1: 76,
  greater_realm_castle_claim_v1: 77,
  greater_realm_cell_occupancy_v1: 78,
  greater_realm_resource_node_v1: 79,
  greater_realm_activation_v1: 80,
  realm_atlas_v1: 81,
  realm_atlas_visible_region_v1: 82,
  realm_worker_system_v2: 83,
  production_player_canary_baseline_v1: 84,
  production_player_canary_approval_registration_v1: 85,
});
...
function assertDeployedV14TablesUnchanged(before, after) {
...
function assertAdditiveV15Schema(before, after) {
  assertDeployedV14TablesUnchanged(before, after);
  const beforeNames = new Set(before.tables.map(table => table.name));
  const added = after.tables
    .map(table => table.name)
    .filter(name => !beforeNames.has(name))
...
function assertDeployedV15TablesUnchanged(before, after) {
  for (const name of deployedV15Tables) {
    assert.deepEqual(tableSignature(after, name), tableSignature(before, name));
    assert.equal(
      tableSignature(after, name).product_type_ref,
      expectedProductTypeRefs[name],
...
function assertAdditiveV16Schema(before, after) {
  assertDeployedV15TablesUnchanged(before, after);
  const beforeNames = new Set(before.tables.map(table => table.name));
  const added = after.tables
    .map(table => table.name)
    .filter(name => !beforeNames.has(name))
    .sort();
  assert.deepEqual(added, [...additiveV16Tables].sort());
  const contracts = {
    realm_chat_status_v1: {
      access: 'Public',
      fields: [
        'channel_key', 'realm_id', 'policy_version', 'mode', 'recent_limit',
        'history_page_limit', 'updated_at',
      ],
    },
    realm_chat_channel_v1: {
      access: 'Private',
    ...
    realm_chat_message_v1: {
      access: 'Private',
      fields: [
        'message_id', 'sequence', 'channel_key', 'sender_fid', 'body',
        'sent_at', 'visibility', 'moderated_at', 'moderation_code',
      ],
    },
    realm_chat_recent_v1: {
      access: 'Private',
    ...
    realm_chat_rate_event_v1: {
      access: 'Private',
      fields: ['event_id', 'fid', 'accepted_at_micros', 'body_digest'],
    },
    realm_chat_send_receipt_v1: {
      access: 'Private',
      fields: [
        'operation_key', 'fid', 'request_key', 'body_digest', 'message_id',
        'sequence', 'created_at',
      ],
    },
    realm_chat_report_v1: {
      access: 'Private',
      fields: [
        'report_ordinal', 'report_key', 'report_id', 'reporter_fid',
        'message_id', 'reported_sender_fid', 'message_sequence', 'category',
...
function assertDeployedV16TablesUnchanged(before, after) {
  for (const name of deployedV16Tables) {
    assert.deepEqual(tableSignature(after, name), tableSignature(before, name));
    assert.equal(
      tableSignature(after, name).product_type_ref,
      expectedProductTypeRefs[name],
...
function assertAdditiveV17Schema(before, after) {
  assertDeployedV16TablesUnchanged(before, after);
  const beforeNames = new Set(before.tables.map(table => table.name));
  const added = after.tables
    .map(table => table.name)
    .filter(name => !beforeNames.has(name))
    .sort();
  assert.deepEqual(added, [...additiveV17Tables].sort());
  const contracts = {
    greater_realm_release_v1: {
      access: 'Private',
      fields: [
        'atlas_id', 'public_release_id', 'public_approval_receipt_id',
        'source_commit', 'generator_version', 'source_format_version',
        'living_world_version', 'runtime_partition_version',
        'renderer_contract_version', 'expected_region_count',
        'expected_component_count', 'expected_chunk_count',
        'expected_cell_count', 'expected_slot_count',
        'expected_resource_node_count', 'component_expected_cell_count',
        'component_expected_slot_count', 'component_expected_resource_node_count',
        'imported_passable_cell_count', 'expected_release_sha_256',
        'release_header_sha_256', 'import_epoch', 'public_name',
        'component_manifest_json', 'region_manifest_json',
        'region_verification_json', 'legacy_transform_rotation',
        'legacy_transform_offset_q', 'legacy_transform_offset_r',
        'verified_legacy_cell_count', 'verified_legacy_water_cell_count',
        'legacy_water_body_verification_json', 'legacy_resource_verification_json',
        'next_chunk_ordinal', 'verification_phase', 'verification_cursor',
        'verification_digest', 'verified_component_count',
        'verified_chunk_count', 'verified_cell_count', 'verified_slot_count',
        'verified_resource_node_count', 'state', 'approved_at', 'staged_at',
        'ready_at',
      ],
    },
    greater_realm_chunk_v1: {
      access: 'Private',
      fields: [
        'chunk_handle', 'atlas_id', 'chunk_coord_key', 'import_ordinal',
        'bin_q', 'bin_r', 'first_cell_ordinal', 'core_cell_count',
        'apron_cell_count', 'lod_0_cell_count', 'lod_1_cell_count',
        'lod_2_cell_count', 'lod_3_cell_count', 'payload_sha_256',
    ...
    greater_realm_navigation_component_v1: {
      access: 'Private',
      fields: [
        'component_key', 'atlas_id', 'component_ordinal', 'region_mask',
        'root_cell_key', 'expected_cell_count', 'imported_cell_count',
        'verified_cell_count', 'verified_region_mask',
        'verified_max_route_depth', 'max_route_depth', 'expected_slot_count',
        'imported_slot_count', 'expected_food_node_count',
        'imported_food_node_count', 'expected_wood_node_count',
        'imported_wood_node_count', 'expected_stone_node_count',
        'imported_stone_node_count', 'expected_gold_node_count',
        'imported_gold_node_count', 'verified_slot_count',
        'verified_food_node_count', 'verified_wood_node_count',
        'verified_stone_node_count', 'verified_gold_node_count',
        'component_sha_256', 'verification_phase', 'verification_digest',
        'region_verification_json', 'active',
      ],
    },
    greater_realm_cell_v1: {
      access: 'Private',
      fields: [
        'cell_key', 'atlas_coord_key', 'release_ordinal', 'atlas_id',
        'chunk_handle', 'region_id', 'component_key', 'local_q', 'local_r',
        'atlas_q', 'atlas_r', 'tier', 'passable', 'elevation', 'slope',
        'aspect', 'profile_curvature', 'plan_curvature', 'ridge_id',
        'geological_barrier_band', 'biome_class', 'landform_class',
        'yield_class', 'movement_cost', 'sealed_boundary_mask',
        'hydro_regime', 'hydro_body_id', 'hydro_depth_class',
        'hydro_surface_milli', 'hydro_flow_direction', 'flow_accumulation',
        'bank_variant', 'hydrology_revision', 'route_parent_direction',
        'route_depth', 'travel_class', 'wetness', 'exposure', 'coast_distance',
        'freshwater_distance', 'temperature', 'moisture', 'habitat_class',
        'canopy_basis_points', 'groundcover_basis_points',
        'wildflower_basis_points', 'feature_class', 'ambience_class',
    ...
    greater_realm_castle_slot_v1: {
      access: 'Private',
      fields: [
        'slot_id', 'release_ordinal', 'atlas_id', 'cell_key', 'region_id',
        'component_key', 'legacy_slot_id', 'tier', 'region_order_rank',
        'allocation_rank', 'active',
      ],
    },
    greater_realm_castle_claim_v1: {
      access: 'Private',
      fields: [
        'slot_id', 'owner_fid', 'castle_id', 'atlas_id', 'activation_id',
        'state', 'claim_kind', 'allocation_sequence', 'planned_at',
        'activated_at', 'legacy_slot_id', 'legacy_claimed_at',
        'legacy_generation_version', 'legacy_tile_key', 'legacy_q', 'legacy_r',
      ],
    },
    greater_realm_cell_occupancy_v1: {
      access: 'Public',
    ...
    greater_realm_resource_node_v1: {
      access: 'Private',
      fields: [
        'node_id', 'release_ordinal', 'atlas_id', 'location_id', 'cell_key',
        'region_id', 'component_key', 'resource_kind', 'tier', 'node_ordinal',
        'allocation_rank', 'legacy_catalog_id', 'policy_version', 'active',
      ],
    },
    greater_realm_activation_v1: {
      access: 'Private',
      fields: [
        'activation_id', 'atlas_id', 'quiet_epoch', 'mode',
        'snapshot_castle_count', 'snapshot_worker_count',
        'snapshot_resource_account_count', 'snapshot_mark_account_count',
        'snapshot_inner_keep_building_count', 'snapshot_claim_count',
        'snapshot_occupancy_count', 'snapshot_castle_digest',
        'snapshot_worker_digest', 'snapshot_resource_digest',
        'snapshot_marks_digest', 'snapshot_inner_keep_digest',
        'snapshot_schedule_digest', 'topology_snapshot_digest',
        'relocation_plan_digest', 'next_allocation_sequence',
        'post_canary_founding_count', 'post_canary_dispatch_count',
        'actor_subject', 'prepared_at', 'draining_at', 'frozen_at', 'planned_at',
        'canary_at', 'activated_at', 'halted_at', 'rolled_back_at',
      ],
    },
    realm_atlas_v1: {
      access: 'Public',
      fields: [
        'atlas_id', 'public_release_id', 'name', 'protocol_version',
        'generator_version', 'runtime_partition_version',
        'renderer_contract_version', 'revision', 'visible_tier_max',
        'navigation_tier_max', 'founding_tier_max', 'visible_region_count',
        'visible_cell_count', 'visible_chunk_count', 'castle_capacity', 'mode',
        'created_at', 'activated_at',
      ],
    },
    realm_atlas_visible_region_v1: {
      access: 'Public',
      fields: [
        'region_id', 'atlas_id', 'ordinal', 'public_name', 'tier', 'cell_count',
        'passable_cell_count', 'chunk_count', 'castle_capacity',
        'resource_location_count', 'resource_node_count', 'food_node_count',
        'wood_node_count', 'stone_node_count', 'gold_node_count', 'active',
      ],
    },
    realm_worker_system_v2: {
      access: 'Public',
      fields: [
        'atlas_id', 'policy_version', 'workers_per_castle', 'castle_capacity',
        'current_castle_count', 'current_worker_count', 'roster_digest', 'mode',
        'created_at', 'activated_at',
      ],
    },
  };
...
function assertDeployedV17TablesUnchanged(before, after) {
  for (const name of deployedV17Tables) {
    assert.deepEqual(tableSignature(after, name), tableSignature(before, name));
    assert.equal(
      tableSignature(after, name).product_type_ref,
      expectedProductTypeRefs[name],
...
function assertCurrentCandidateSchema(before, after) {
  assertDeployedV17TablesUnchanged(before, after);
  const beforeNames = new Set(before.tables.map(table => table.name));
  const added = after.tables
    .map(table => table.name)
    .filter(name => !beforeNames.has(name))
    .sort();
  assert.deepEqual(added, [...productionPlayerCanaryTables].sort());
  const contracts = {
    production_player_canary_baseline_v1: {
      access: 'Private',
      fields: [
        'challenge_digest', 'fid', 'reviewed_admission_plan_digest',
        'baseline_commitment', 'route_set_commitment', 'castle_id', 'atlas_id',
        'atlas_revision', 'captured_at', 'resource_settled_through_micros',
        'resource_revision', 'resource_food', 'resource_wood', 'resource_stone',
        'resource_gold', 'resource_policy_version',
        'resource_created_at_micros', 'pristine_roster_commitment',
      ],
    },
    production_player_canary_approval_registration_v1: {
      access: 'Private',
      fields: [
        'challenge_digest', 'fid', 'reviewed_admission_plan_digest',
        'server_baseline_commitment', 'route_set_commitment',
        'command_key_policy_version', 'command_set_commitment',
        'owner_approval_artifact_digest', 'owner_approval_commitment',
        'approval_registration_commitment', 'approved_at_micros',
        'not_after_micros', 'registered_at',
      ],
    },
  };
  for (const [name, contract] of Object.entries(contracts)) {
...
function assertCurrentCandidateTablesUnchanged(before, after) {
  for (const name of deployedCurrentCandidateTables) {
    assert.deepEqual(tableSignature(after, name), tableSignature(before, name));
    assert.equal(
      tableSignature(after, name).product_type_ref,
      expectedProductTypeRefs[name],
...
async function verifyActualModuleResourceLifecycle(server, database, privateKey, ownerToken) {
  ...
  // Keep inspection on the complete current candidate schema. Reverting to
  // frozen v17 after the private canary suffix is appended is destructive.
  const inspectionArtifactPath = join(currentCandidateInspectionFixture, 'dist', 'bundle.js');
...
async function verifyActualModuleExpeditionLifecycles(
  ...
  let activeModule = 'actual';
  const actualArtifactPath = join(additiveModule, 'dist', 'bundle.js');
  // Reusing the current-candidate fixture preserves the complete private
  // canary suffix during SQL inspection; any predecessor is a downgrade.
  const inspectionArtifactPath = join(currentCandidateInspectionFixture, 'dist', 'bundle.js');
  const useActualModule = async () => {
    if (activeModule === 'actual') return;
    await publishBuiltArtifact(server, ownerToken, actualArtifactPath, database);
...
async function main() {
  ...
  try {
    ...
    // Populate both private v14 tables before the v15 append. This freezes the
    // actual predecessor boundary rather than proving only empty-table shape.
    await callLoopbackReducer(
      server,
      dailyMarksMigrationDatabase,
      'fixture_seed_daily_marks_sentinel_v14',
      owner.token,
      '[]',
      200,
    );
    const dailyMarksPopulatedV14Rows = await tableRowDigests(
      server,
      owner.token,
      dailyMarksMigrationDatabase,
      deployedV14Tables,
    ...
    for (const table of additiveV14Tables) {
      assert.equal(await count(server, owner.token, dailyMarksMigrationDatabase, table), 1n);
    }
    ...
    const dailyMarksV15 = await describe(
      server,
      owner.token,
      dailyMarksMigrationDatabase,
    );
    assertAdditiveV15Schema(dailyMarksV14, dailyMarksV15);
    assert.deepEqual(
      await tableRowDigests(
        server,
        owner.token,
      ...
      dailyMarksPopulatedV14Rows,
    );
    for (const table of additiveV15Tables) {
      assert.equal(await count(server, owner.token, dailyMarksMigrationDatabase, table), 0n);
    }
    await callLoopbackReducer(
      server,
      dailyMarksMigrationDatabase,
      'fixture_seed_inner_keep_sentinel_v15',
      owner.token,
      '[]',
      200,
    );
    const dailyMarksPopulatedV15Rows = await tableRowDigests(
      server,
      owner.token,
      dailyMarksMigrationDatabase,
      deployedV15Tables,
    );
    const populatedDailyMarksV15SchemaDigest = schemaDigest(dailyMarksV15);
    await publish(
      server,
      owner.token,
      additiveV15SchemaFixture,
    ...
    assert.equal(
      schemaDigest(await describe(server, owner.token, dailyMarksMigrationDatabase)),
      populatedDailyMarksV15SchemaDigest,
    );
    assert.deepEqual(
      await tableRowDigests(
        server,
    ...
    // Freeze v15 independently before the real candidate. The complete v14
    // predecessor remains unchanged and all eight Inner Keep tables start
    // empty on every representative database.
    await publish(server, owner.token, additiveV15SchemaFixture, emptyDatabase);
    await publish(server, owner.token, additiveV15SchemaFixture, nonemptyDatabase);
    await publish(server, owner.token, additiveV15SchemaFixture, actualModuleDatabase);
    await publish(server, owner.token, additiveV15SchemaFixture, resourceLifecycleDatabase);
    await publish(
      server,
      owner.token,
      additiveV15SchemaFixture,
    ...
    const emptyV15 = await describe(server, owner.token, emptyDatabase);
    const nonemptyV15 = await describe(server, owner.token, nonemptyDatabase);
    const actualModuleV15 = await describe(server, owner.token, actualModuleDatabase);
    const populatedWaterStoneV15 = await describe(
      server,
      owner.token,
      populatedWaterStoneMigrationDatabase,
    );
    assertAdditiveV15Schema(emptyV14, emptyV15);
    assertAdditiveV15Schema(nonemptyV14, nonemptyV15);
    assertAdditiveV15Schema(actualModuleV14, actualModuleV15);
    assertAdditiveV15Schema(populatedWaterStoneV14, populatedWaterStoneV15);
    const fixtureV15TableSchemaDigest = canonicalTableSchemaBoundaryDigest(
      emptyV15,
      deployedV15Tables,
    );
    for (const description of [nonemptyV15, actualModuleV15, populatedWaterStoneV15]) {
      assert.equal(
        canonicalTableSchemaBoundaryDigest(description, deployedV15Tables),
        fixtureV15TableSchemaDigest,
      );
    }
    for (const database of [
      emptyDatabase,
    ...
    await publish(server, owner.token, additiveV16SchemaFixture, actualModuleDatabase);
    await publish(server, owner.token, additiveV16SchemaFixture, resourceLifecycleDatabase);
    await publish(
      server,
      owner.token,
    ...
    const emptyV16 = await describe(server, owner.token, emptyDatabase);
    const nonemptyV16 = await describe(server, owner.token, nonemptyDatabase);
    const actualModuleV16 = await describe(server, owner.token, actualModuleDatabase);
    const populatedWaterStoneV16 = await describe(
      server,
      owner.token,
      populatedWaterStoneMigrationDatabase,
    );
    assertAdditiveV16Schema(emptyV15, emptyV16);
    assertAdditiveV16Schema(nonemptyV15, nonemptyV16);
    assertAdditiveV16Schema(actualModuleV15, actualModuleV16);
    assertAdditiveV16Schema(populatedWaterStoneV15, populatedWaterStoneV16);
    const fixtureV16TableSchemaDigest = canonicalTableSchemaBoundaryDigest(
      emptyV16,
      deployedV16Tables,
    );
    for (const description of [nonemptyV16, actualModuleV16, populatedWaterStoneV16]) {
      assert.equal(
        canonicalTableSchemaBoundaryDigest(description, deployedV16Tables),
        fixtureV16TableSchemaDigest,
      );
    }
    for (const database of [
      emptyDatabase,
      ...
      for (const table of additiveV16Tables) {
        assert.equal(await count(server, owner.token, database, table), 0n);
      }
    }

    // A dedicated database proves the populated v15 -> v16 Chat append and
    // refuses the immediate downgrade before any Chat state can be removed.
    await publish(server, owner.token, additiveV15SchemaFixture, realmChatMigrationDatabase);
    const realmChatV15 = await describe(server, owner.token, realmChatMigrationDatabase);
    await publish(server, owner.token, additiveV16SchemaFixture, realmChatMigrationDatabase);
    const realmChatV16 = await describe(server, owner.token, realmChatMigrationDatabase);
    assertAdditiveV16Schema(realmChatV15, realmChatV16);
    for (const table of additiveV16Tables) {
      assert.equal(await count(server, owner.token, realmChatMigrationDatabase, table), 0n);
    }
    await callLoopbackReducer(
      server,
      realmChatMigrationDatabase,
      'fixture_seed_realm_chat_sentinel_v16',
      owner.token,
      '[]',
      200,
    );
    const populatedRealmChatV16Rows = await tableRowDigests(
      server,
      owner.token,
      realmChatMigrationDatabase,
      additiveV16Tables,
    ...
    for (const table of additiveV16Tables) {
      assert.equal(await count(server, owner.token, realmChatMigrationDatabase, table), 1n);
    }
    const populatedRealmChatV16SchemaDigest = schemaDigest(realmChatV16);
    await publish(
      server,
      owner.token,
      additiveV15SchemaFixture,
    ...
    assert.equal(
      schemaDigest(await describe(server, owner.token, realmChatMigrationDatabase)),
      populatedRealmChatV16SchemaDigest,
    );
    assert.deepEqual(
      await tableRowDigests(server, owner.token, realmChatMigrationDatabase, additiveV16Tables),
      populatedRealmChatV16Rows,
    );

    // Build one populated, exact v16 predecessor carrying representative
    // legacy Water, Inner Keep, and Chat rows. The v17 proof hashes every one
    // of the 72 predecessor tables before any Greater Realm table exists.
    await publish(
      server,
      owner.token,
      additiveV16SchemaFixture,
      greaterRealmMigrationDatabase,
    );
    const greaterRealmV16 = await describe(
      server,
      owner.token,
      greaterRealmMigrationDatabase,
    );
    for (const name of deployedV16Tables) {
      assert.deepEqual(tableSignature(greaterRealmV16, name), tableSignature(emptyV16, name));
    }
    await callLoopbackReducer(
      server,
      greaterRealmMigrationDatabase,
    ...
    const populatedGreaterRealmPredecessorV16Rows = await tableRowDigests(
      server,
      owner.token,
      greaterRealmMigrationDatabase,
      deployedV16Tables,
    );
    // Freeze the exact v17 suffix independently before installing the real
    // artifact. Every representative database must expose refs 72-83 with the
    // fixture's complete access, index, constraint, field-order, and type ABI.
    for (const database of [
      emptyDatabase,
      nonemptyDatabase,
      actualModuleDatabase,
      ...
      await publish(server, owner.token, additiveV17SchemaFixture, database);
    }
    const emptyV17 = await describe(server, owner.token, emptyDatabase);
    const nonemptyV17 = await describe(server, owner.token, nonemptyDatabase);
    const actualModuleV17 = await describe(server, owner.token, actualModuleDatabase);
    const populatedWaterStoneV17 = await describe(
      server,
      owner.token,
      populatedWaterStoneMigrationDatabase,
    );
    const greaterRealmFixtureV17 = await describe(
      server,
      owner.token,
      greaterRealmMigrationDatabase,
    );
    assertAdditiveV17Schema(emptyV16, emptyV17);
    assertAdditiveV17Schema(nonemptyV16, nonemptyV17);
    assertAdditiveV17Schema(actualModuleV16, actualModuleV17);
    assertAdditiveV17Schema(populatedWaterStoneV16, populatedWaterStoneV17);
    assertAdditiveV17Schema(greaterRealmV16, greaterRealmFixtureV17);
    const fixtureV17TableSchemaDigest = canonicalTableSchemaBoundaryDigest(
      emptyV17,
      deployedV17Tables,
    );
    for (const description of [
      nonemptyV17,
      actualModuleV17,
      populatedWaterStoneV17,
      greaterRealmFixtureV17,
    ]) {
      assert.equal(
        canonicalTableSchemaBoundaryDigest(description, deployedV17Tables),
        fixtureV17TableSchemaDigest,
      );
    }
    assert.deepEqual(
      await tableRowDigests(
      ...
      populatedGreaterRealmPredecessorV16Rows,
    );
    for (const database of [
      emptyDatabase,
      nonemptyDatabase,
      ...
      for (const table of additiveV17Tables) {
        assert.equal(await count(server, owner.token, database, table), 0n);
      }
    }

    // Advance every database to the real current candidate so both private
    // canary tables are exercised in the append-only contract without production.
    ...
    await publish(server, owner.token, additiveModule, dailyMarksMigrationDatabase);
    await publish(server, owner.token, additiveModule, realmChatMigrationDatabase);
    ...
    const populatedWaterStoneCandidateV17 = await describe(
      server,
      owner.token,
      populatedWaterStoneMigrationDatabase,
    );
    const dailyMarksCandidateV17 = await describe(
      server,
      owner.token,
      dailyMarksMigrationDatabase,
    );
    const realmChatCandidateV17 = await describe(
      server,
      owner.token,
      realmChatMigrationDatabase,
    );
    const greaterRealmCandidateV17 = await describe(
      server,
      owner.token,
      greaterRealmMigrationDatabase,
    );
    for (const description of [
      populatedWaterStoneCandidateV17,
      dailyMarksCandidateV17,
      realmChatCandidateV17,
      greaterRealmCandidateV17,
    ]) {
    ...
    const dailyMarksInspectionCandidate = await describe(
      server,
      owner.token,
      dailyMarksMigrationDatabase,
    );
    assertCurrentCandidateTablesUnchanged(
      dailyMarksCandidateV17,
      dailyMarksInspectionCandidate,
    );
    assert.deepEqual(
      await tableRowDigests(
        server,
    ...
    const realmChatInspectionCandidate = await describe(
      server,
      owner.token,
      realmChatMigrationDatabase,
    );
    ...
    const greaterRealmInspectionCandidate = await describe(
      server,
      owner.token,
      greaterRealmMigrationDatabase,
    );
    assertCurrentCandidateTablesUnchanged(
      greaterRealmCandidateV17,
      greaterRealmInspectionCandidate,
    );
    assert.deepEqual(
      await tableRowDigests(
        server,
    ...
    await callLoopbackReducer(
      server,
      greaterRealmMigrationDatabase,
      'fixture_seed_greater_realm_sentinel_v17',
      owner.token,
      '[]',
      200,
    );
    const populatedGreaterRealmV17Rows = await tableRowDigests(
      server,
      owner.token,
      greaterRealmMigrationDatabase,
      additiveV17Tables,
    ...
    for (const table of additiveV17Tables) {
      assert.equal(await count(server, owner.token, greaterRealmMigrationDatabase, table), 1n);
    }
    for (const table of productionPlayerCanaryTables) {
      assert.equal(await count(server, owner.token, greaterRealmMigrationDatabase, table), 0n);
    }
    const populatedGreaterRealmV17SchemaDigest = schemaDigest(
      await describe(server, owner.token, greaterRealmMigrationDatabase),
    );

    // The typed suffix survives the real artifact, and publishing that same
    // artifact twice remains idempotent. Switch back only to the exact,
    // schema-identical current-candidate fixture for private SQL verification.
    await publishBuiltArtifact(
      server,
      owner.token,
      builtArtifactPath,
    ...
    const populatedGreaterRealmCandidateV17 = await describe(
      server,
      owner.token,
      greaterRealmMigrationDatabase,
    );
    assertCurrentCandidateSchema(emptyV17, populatedGreaterRealmCandidateV17);
    await publishBuiltArtifact(
      server,
      owner.token,
      builtArtifactPath,
    ...
    assert.equal(
      schemaDigest(await describe(server, owner.token, greaterRealmMigrationDatabase)),
      schemaDigest(populatedGreaterRealmCandidateV17),
    );
    await publish(
      server,
      owner.token,
    ...
    assertCurrentCandidateTablesUnchanged(
      populatedGreaterRealmCandidateV17,
      await describe(server, owner.token, greaterRealmMigrationDatabase),
    );
    assert.deepEqual(
      await tableRowDigests(
    ...
    assert.equal(
      schemaDigest(await describe(server, owner.token, greaterRealmMigrationDatabase)),
      populatedGreaterRealmV17SchemaDigest,
    );
    assert.deepEqual(
      await tableRowDigests(
        server,
    ...
    const workerRolloutCandidateV15Fixture = await describe(
      server,
      owner.token,
      workerRolloutV11Database,
    );
    assertAdditiveV15Schema(workerRolloutCandidateV14, workerRolloutCandidateV15Fixture);
    await publish(
      server,
      owner.token,
      additiveV16SchemaFixture,
    ...
    const workerRolloutCandidateV16Fixture = await describe(
      server,
      owner.token,
      workerRolloutV11Database,
    );
    assertAdditiveV16Schema(workerRolloutCandidateV15Fixture, workerRolloutCandidateV16Fixture);
    await publish(
      server,
      owner.token,
    ...
    const workerRolloutCandidateV17Fixture = await describe(
      server,
      owner.token,
      workerRolloutV11Database,
    );
    assertAdditiveV17Schema(workerRolloutCandidateV16Fixture, workerRolloutCandidateV17Fixture);
    await publishBuiltArtifact(
      server,
      owner.token,
      builtArtifactPath,
    ...
    const workerRolloutCandidateV17 = await describe(
      server,
      owner.token,
      workerRolloutV11Database,
    );
    assertCurrentCandidateSchema(workerRolloutCandidateV17Fixture, workerRolloutCandidateV17);
    // The real module rejects the disposable owner identity by design. Swap
    // only this database to the schema-identical current-candidate fixture
    ...
    assertCurrentCandidateTablesUnchanged(
      workerRolloutCandidateV17,
      await describe(server, owner.token, workerRolloutV11Database),
    );
    assert.deepEqual(
      await tableRowDigests(
        server,
    ...
    const builtArtifactDigest = createHash('sha256')
      ...
      .digest('hex');
    const emptyCandidateV17 = await describe(server, owner.token, emptyDatabase);
    const nonemptyCandidateV17 = await describe(server, owner.token, nonemptyDatabase);
    const actualCandidateV17 = await describe(server, owner.token, actualModuleDatabase);
    for (const description of [
      emptyCandidateV17,
      nonemptyCandidateV17,
      actualCandidateV17,
    ]) {
      assertCurrentCandidateSchema(emptyV17, description);
    }
    ...
    const provenV14TableSchemaDigest = projectedTableSchemaBoundaryDigest(
      emptyCandidateV17,
      deployedV14Tables,
    );
    assert.equal(provenV14TableSchemaDigest, fixtureV14TableSchemaDigest);
    const provenV15TableSchemaDigest = projectedTableSchemaBoundaryDigest(
      emptyCandidateV17,
      deployedV15Tables,
    );
    assert.equal(provenV15TableSchemaDigest, fixtureV15TableSchemaDigest);
    const provenV16TableSchemaDigest = projectedTableSchemaBoundaryDigest(
      emptyCandidateV17,
      deployedV16Tables,
    );
    assert.equal(provenV16TableSchemaDigest, fixtureV16TableSchemaDigest);
    const provenV17TableSchemaDigest = projectedTableSchemaBoundaryDigest(
      emptyCandidateV17,
      deployedV17Tables,
    );
    assert.equal(provenV17TableSchemaDigest, fixtureV17TableSchemaDigest);
    const provenCurrentCandidateTableSchemaDigest =
      canonicalTableSchemaBoundaryDigest(
        emptyCandidateV17,
        deployedCurrentCandidateTables,
      );
    for (const description of [nonemptyCandidateV17, actualCandidateV17]) {
      assert.equal(
      ...
      assert.equal(
        projectedTableSchemaBoundaryDigest(description, deployedV14Tables),
        provenV14TableSchemaDigest,
      );
      assert.equal(
        projectedTableSchemaBoundaryDigest(description, deployedV15Tables),
        provenV15TableSchemaDigest,
      );
      assert.equal(
        projectedTableSchemaBoundaryDigest(description, deployedV16Tables),
        provenV16TableSchemaDigest,
      );
      assert.equal(
        projectedTableSchemaBoundaryDigest(description, deployedV17Tables),
        provenV17TableSchemaDigest,
      );
      assert.equal(
        canonicalTableSchemaBoundaryDigest(
          description,
        ...
        provenCurrentCandidateTableSchemaDigest,
      );
    }
    for (const name of deployedV12Tables) {
      assert.deepEqual(
        tableSignature(actualCandidateV17, name),
        tableSignature(emptyV12, name),
      );
      assert.deepEqual(
        tableSignature(nonemptyCandidateV17, name),
    ...
    // owner identity. Reuse the table-identical, auth-neutral current-candidate
    // fixture before owner SQL reads and never downgrade the private suffix.
    await publish(server, owner.token, currentCandidateInspectionFixture, emptyDatabase);
    await publish(server, owner.token, currentCandidateInspectionFixture, nonemptyDatabase);
    await publish(server, owner.token, currentCandidateInspectionFixture, actualModuleDatabase);
    for (const [candidate, database] of [
      [emptyCandidateV17, emptyDatabase],
      [nonemptyCandidateV17, nonemptyDatabase],
      [actualCandidateV17, actualModuleDatabase],
    ]) {
      assertCurrentCandidateTablesUnchanged(
        candidate,
        await describe(server, owner.token, database),
      );
    }
    // SQL preservation reads remain on the complete current candidate. No
    // reducer is invoked by these owner-only queries.
    ...
    const idempotentSchemaBefore = schemaDigest(nonemptyCandidateV17);
    await publishBuiltArtifact(
      server,
      owner.token,
      builtArtifactPath,
    ...
    assertCurrentCandidateTablesUnchanged(
      nonemptyCandidateV17,
      await describe(server, owner.token, nonemptyDatabase),
    );

    // The actual module correctly rejects the disposable local identity at its
    // on-connect boundary; owner SQL still reads the unchanged candidate rows.
    assert.equal(await count(server, owner.token, emptyDatabase, 'player'), 0n);
    ...
    // issuer boundary is active. The auth-neutral current-candidate fixture
    // inserts the verified sender identity through a disposable reducer.
    await callLoopbackReducer(
      server,
      emptyDatabase,
    ...
    const populatedCurrentCandidateSchemaDigest = schemaDigest(
    ...
    await assertFixtureOwnershipCount(server, owner.token, emptyDatabase, 999999, 1);
    const populatedCurrentCandidateRowsBeforeRollback = await tableRowDigests(
      server,
      owner.token,
      emptyDatabase,
      deployedCurrentCandidateTables,
    ...
    // The current candidate first refuses its exact frozen v17 predecessor.
    // Deletion stays disabled and neither canary table can be removed.
    await publish(
      server,
      owner.token,
      additiveV17SchemaFixture,
    ...
    assert.equal(
      ...
      populatedCurrentCandidateSchemaDigest,
    );
    assert.deepEqual(
      await tableRowDigests(
        server,
      ...
      populatedCurrentCandidateRowsBeforeRollback,
    );

    // The frozen v17 boundary also refuses its exact v16 predecessor. This is
    // a rejected-publish probe: no predecessor row or suffix byte may change.
    await publish(
      server,
      owner.token,
      additiveV16SchemaFixture,
    ...
    // The v16 boundary must refuse its immediate predecessor before any Chat
    ...
    // The v15 boundary must refuse its own predecessor before any
    // Inner Keep state can be removed.
    await publish(
      server,
      owner.token,
      additiveV14SchemaFixture,
    ...
    // The v14 boundary must also refuse its own predecessor before either
    ...
    await publish(server, owner.token, additiveModule, emptyDatabase);
    const finalCurrentCandidate = await describe(server, owner.token, emptyDatabase);
    assertCurrentCandidateSchema(emptyV17, finalCurrentCandidate);
    // Reuse the table-identical auth-neutral current-candidate fixture for the
    // final bounded identity assertion; the real module rejects the disposable
    // owner issuer before any private identity SQL can run.
    await publish(server, owner.token, currentCandidateInspectionFixture, emptyDatabase);
    assertCurrentCandidateTablesUnchanged(
    ...
    console.log(formatAdditiveMigrationProofReceipt({
      ...
      + 'private Greater Realm release, chunk, navigation, cell, founding, resource, and activation authority plus public occupancy, atlas, region, and worker readiness appended at exact refs 72-83, '
      + 'private production-player baseline and approval-registration authority appended at exact refs 84-85, '
      + '61-tile empty, synthetic nonempty, and populated Water/Stone/Water-revision fixtures remained preserved through v17, '
      + 'every v12 table was populated and retained through the real candidate, the v13 request suffix survived populated, '
      + 'both v14 tables, all eight v15 tables, all eight v16 tables, all twelve v17 tables, and both private canary tables started empty, fixture republish remained idempotent, '
      + 'representative legacy Water, Inner Keep, and Chat predecessor rows were byte-preserved, every typed v17 sentinel survived two real-artifact publishes, '
      + 'and the complete state was protected from current-candidate-to-v17, v17-to-v16, and older downgrades, '
      ...
      + 'prebuilt-artifact republish idempotent, populated v3-prefix state retained through the current candidate, '
      + 'and guarded v16/v15/v14/v13/v12/v11/v10/v9/v8/v7/v6/v5/v4/v3/v2 rollbacks refused before schema change.',
      ...
      v15TableSchemaDigest: provenV15TableSchemaDigest,
      v16TableSchemaDigest: provenV16TableSchemaDigest,
      v17TableSchemaDigest: provenV17TableSchemaDigest,
      currentCandidateTableSchemaDigest: provenCurrentCandidateTableSchemaDigest,
