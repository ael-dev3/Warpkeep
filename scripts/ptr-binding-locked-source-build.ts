...
const PROVENANCE_DOMAIN = 'warpkeep-ptr-independent-dependency-closure-v1';
...
const EXPECTED_PACKAGE_EDGES = Object.freeze<Record<string, readonly string[]>>({
...
function selectedPackages(lockBody: Buffer): readonly LockedPackage[] {
  ...
  while (pending.length > 0) {
    ...
    const platform = platformMetadata(packageRecord);
    ...
    for (const field of ['dependencies', 'optionalDependencies'] as const) {
      ...
      for (const [dependencyName, dependencyVersion] of Object.entries(edges)) {
        ...
        const dependencyPlatform = platformMetadata(dependencyRecord);
  ...
  if (JSON.stringify(packages.map(value => value.key)) !== JSON.stringify(EXPECTED_PACKAGE_KEYS)) {
    for (const value of packages) value.integrityDigest.fill(0);
    fail(code);
  }
  for (const package_ of packages) {
    if (JSON.stringify(package_.dependencies) !== JSON.stringify(EXPECTED_PACKAGE_EDGES[package_.key])) {
      for (const value of packages) value.integrityDigest.fill(0);
      fail(code);
    }
  }
...
function dependencyClosureDigest(input: Readonly<{
  ...
  updateFramed(digest, 'domain', PROVENANCE_DOMAIN);
...
export function withPtrLockedSourceBuild<T>(input: Readonly<{
...
}>): Readonly<{ result: T; dependencyClosureDigest: string; moduleTreeId: string }> {
  ...
  try {
    ...
    packages = selectedPackages(lock.body);
    ...
    closureDigest = dependencyClosureDigest({
      manifestBytes: manifest.body, lockBytes: lock.body, packages, snapshot: installedSnapshot,
    });
