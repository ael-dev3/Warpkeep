...
const UID = 1001;
const ACCOUNT_HOME = '/home/runner';
...
function runtime(expected) {
  ...
  if (account.uid !== UID || account.gid !== UID || account.username !== 'runner' || account.homedir !== ACCOUNT_HOME) fail('runtime');
  directory(ACCOUNT_HOME, UID, 0o700);
  ...
  const nodeParent = dirname(process.execPath);
  if (basename(process.execPath) !== 'node' || dirname(nodeParent) !== temporary
    || !/^warpkeep-sealed-preflight-node\.[A-Za-z0-9]{6}$/u.test(basename(nodeParent))) fail('runtime');
  directory(nodeParent, UID, 0o700);
  const nodeStatus = lstatSync(process.execPath);
  if (nodeStatus.gid !== UID || (nodeStatus.mode & 0o7777) !== 0o700) fail('runtime');
  const node = readLocalBindingBoundedFile(process.execPath, { maximumBytes: NODE_BYTES,
    expectedBytes: NODE_BYTES, expectedSha256: NODE_SHA256, expectedUid: UID,
    expectedMode: 0o700, requireExecutable: true, discardBody: true, expectedIdentity: expected?.node });
  parents(GIT, 'runtime');
...
function bundle(commit) {
  ...
  const selected = manifest.bundles.find(item => item.lane === 'g001');
  const spec = getSealedRealmOperationBundleSpecification('g001');
  if (!Array.isArray(selected.graphManifest) || selected.graphManifest.length < 1 || selected.graphManifest.length > 256
    || selected.sourceClosureDigest !== deriveSealedRealmOperationBundleSourceClosureDigest('g001', selected.graphManifest)
  ...
  for (const member of selected.graphManifest) {
    ...
    if (typeof member.path !== 'string' || !/^(?:scripts|spacetimedb|node_modules\/yaml)\/[A-Za-z0-9._/-]+$/u.test(member.path)
    ...
    if (member.path.startsWith('node_modules/')) {
      ...
      const pinned = yaml.files.filter(file => file.path === member.path.slice('node_modules/yaml/'.length));
...
export async function runSealedRealmsProductionLinuxPreflight(input) {
  ...
  if (arguments.length !== 1 || options.operation !== 'preflight'
  ...
  try {
    ...
    phase = 'bundle';
    const selected = bundle(options.workflowInputSha);
    ...
    if (JSON.stringify(Object.keys(loaded).sort()) !== JSON.stringify([...selected.exportNames].sort())
      || typeof loaded.createSealedRealmsProductionG001WorkflowRuntime !== 'function'
      || typeof loaded.runSealedRealmsProductionG001Operation !== 'function') fail('bundle');
    runtime(host); source(options.workflowInputSha); bundle(options.workflowInputSha);
    phase = 'workflow';
    const operation = 'preflight';
    const workflowInputSha = options.workflowInputSha;
    const opaque = await loaded.createSealedRealmsProductionG001WorkflowRuntime({ operation, workflowInputSha });
    runtime(host); source(workflowInputSha); bundle(workflowInputSha);
    const result = await loaded.runSealedRealmsProductionG001Operation({ runtime: opaque, operation, workflowInputSha });
    phase = 'result';
    runtime(host); source(workflowInputSha); bundle(workflowInputSha);
    exact(result, ['operation', 'status'], 'result');
    if (result.operation !== 'preflight' || result.status !== 'preflight-inspected') fail('result');
    return Object.freeze({ operation: 'preflight', status: 'preflight-inspected' });
...
if (process.argv[1] !== undefined && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  let options;
  try {
    if (process.argv.length !== 4 || process.argv[2] !== '--operation=preflight'
      || !/^--source=[a-f0-9]{40}$/u.test(process.argv[3])) fail('input');
    options = { operation: 'preflight', workflowInputSha: process.argv[3].slice('--source='.length) };
  } catch {
    process.stderr.write('{"operation":"preflight","status":"failed","phase":"input"}\n');
    process.exitCode = 1;
  }
  if (options) runSealedRealmsProductionLinuxPreflight(options).then(value => {
    process.stdout.write(`${JSON.stringify(value)}\n`);
  }).catch(error => {
    process.stderr.write(`${JSON.stringify({ operation: 'preflight', status: 'failed', phase: error.phase })}\n`);
    process.exitCode = 1;
  });
}
