...
const retiredAdmissionRequestRuntimeAssets = Object.freeze([
  'public/audio/Hegemony_Empire_Admission_Request_Button.mp3'
]);
...
for (const relativePath of retiredAdmissionRequestRuntimeAssets) {
  if (lstatSync(resolve(root, relativePath), { throwIfNoEntry: false })) {
    throw new Error(`${relativePath} is a retired admission-request sound and must remain absent from production source.`);
  }
}
