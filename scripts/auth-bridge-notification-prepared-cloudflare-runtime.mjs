...
export function parseAuthBridgeNotificationPreparedMultipart(body, contentType) {
  ...
  while (true) {
    ...
    const bytes = Buffer.from(body.subarray(offset, next));
...
function exactMultipartMetadata(metadata, contract) {
  if (
    !isRecord(metadata)
...
function projectVersion(value, contract, sourceDigest) {
  ...
  if (
    ...
    || nonDurableExports[0][1]?.type !== 'worker'
  ) fail('AUTH_BRIDGE_PREPARED_CLOUDFLARE_VERSION_EXPORT_MISMATCH');
  return Object.freeze({
    ...contract,
    versionId: value.id,
...
function exactDomain(result, contract) {
  if (
 ...
 * direct fetch call; this runtime never retries a POST.
 */
export function createAuthBridgeNotificationPreparedCloudflareRuntime({
  ...
  const attestUploadPrerequisites = async () => {
    const [scripts, secrets] = await Promise.all([
      api.json(`${API_PREFIX}/accounts/${contract.accountId}/workers/scripts`),
      api.json(`${basePath}/secrets`),
    ]);
    if (!Array.isArray(scripts.result) || !Array.isArray(secrets.result)) {
      fail('AUTH_BRIDGE_PREPARED_CLOUDFLARE_UPLOAD_PREREQUISITES_INVALID');
    }
    const matches = scripts.result.filter(script => script?.id === contract.workerName);
    if (
      matches.length !== 1
      || matches[0].migration_tag !== contract.migrations.at(-1).tag
  ...
  const reconcileVersion = async () => {
    ...
    if (candidates.length === 0 && phase === 'upload-invoked') {
      fail('AUTH_BRIDGE_PREPARED_DEPLOY_UPLOAD_OUTCOME_AMBIGUOUS', true);
    }
  ...
  const inspectDeployment = async () => {
    const [deploymentResponse, domainResponse, subdomainResponse] = await Promise.all([
      api.json(`${basePath}/deployments`),
      api.json(
        `${API_PREFIX}/accounts/${contract.accountId}/workers/domains?hostname=${encodeURIComponent(contract.route.pattern)}&service=${contract.workerName}&zone_id=${contract.zoneId}`,
      ),
      api.json(`${basePath}/subdomain`),
    ]);
    const latest = exactDeployment(deploymentResponse.result);
    exactDomain(domainResponse.result, contract);
    if (
  ...
  return Object.freeze({
    ...
    releaseVersion,
    inspectDeployment,
    dispose() { preparedMultipart?.body.fill(0); },
  });
}
