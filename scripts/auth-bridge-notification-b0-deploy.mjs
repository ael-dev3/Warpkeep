...
export class AuthBridgeNotificationB0DeployEntrypointError extends Error {
  constructor(code) {
    super(code);
    this.name = 'AuthBridgeNotificationB0DeployEntrypointError';
    this.code = code;
  }
}

function fail(code) {
  throw new AuthBridgeNotificationB0DeployEntrypointError(code);
}
...
function copyAndScrubEnvironment(environment) {
  ...
  for (const name of REQUIRED_ENVIRONMENT) {
    if (typeof environment[name] !== 'string') {
      fail('AUTH_BRIDGE_NOTIFICATION_B0_DEPLOY_ENVIRONMENT_INVALID');
    }
    values[name] = environment[name];
  }
  for (const name of FORBIDDEN_ENVIRONMENT) {
    if (environment[name] !== undefined) {
      fail('AUTH_BRIDGE_NOTIFICATION_B0_DEPLOY_ENVIRONMENT_FORBIDDEN');
    }
  }
  for (const name of [
    'GITHUB_TOKEN',
...
export async function attestAuthBridgeNotificationB0DeployCheckout({
  ...
  const repository = canonicalDirectory(
    repositoryRoot,
    'AUTH_BRIDGE_NOTIFICATION_B0_DEPLOY_REPOSITORY_INVALID',
  );
  ...
  ) fail('AUTH_BRIDGE_NOTIFICATION_B0_DEPLOY_CHECKOUT_MISMATCH');
  return repository;
}
...
async function boundedGithubJson(response) {
  ...
  ) fail('AUTH_BRIDGE_NOTIFICATION_B0_DEPLOY_GITHUB_RESPONSE_INVALID');
...
export function createAuthBridgeNotificationB0GithubWritePermit({
  ...
  fetchImpl = fetch,
  isInterrupted = () => false,
  attestCheckout = attestAuthBridgeNotificationB0DeployCheckout,
} = {}) {
  if (
  ...
  ) fail('AUTH_BRIDGE_NOTIFICATION_B0_DEPLOY_WRITE_PERMIT_INPUT_INVALID');
  const request = async path => {
    ...
    try {
      ...
      fail('AUTH_BRIDGE_NOTIFICATION_B0_DEPLOY_WRITE_PERMIT_UNAVAILABLE');
    } finally {
      clearTimeout(timer);
    }
...
export async function runAuthBridgeNotificationB0Deploy({
  ...
  const repository = await attestAuthBridgeNotificationB0DeployCheckout({
    repositoryRoot: repositoryRoot ?? inferredRepository,
    sourceCommit,
  });
  if (repository !== inferredRepository) {
    fail('AUTH_BRIDGE_NOTIFICATION_B0_DEPLOY_REPOSITORY_INVALID');
  }
  ...
  if (toolchain.wranglerEntrypoint !== realpathSync(exactWranglerEntrypoint)) {
    fail('AUTH_BRIDGE_NOTIFICATION_B0_DEPLOY_TOOLCHAIN_INVALID');
  }
  ...
  ) fail('AUTH_BRIDGE_NOTIFICATION_B0_DEPLOY_SOURCE_CLOSURE_INVALID');
  ...
  const {
    buildAuthBridgeNotificationB0WranglerMultipart,
    createAuthBridgeNotificationB0CloudflareRuntime,
  } = cloudflareRuntime;
  const { withAuthBridgeNotificationB0DeployJournal } = deployJournal;
  let interrupted = false;
  const interrupt = () => { interrupted = true; };
  process.once('SIGINT', interrupt);
  process.once('SIGTERM', interrupt);
  try {
    const permit = createAuthBridgeNotificationB0GithubWritePermit({
    ...
    const withB0Journal = (contract, operation) =>
    ...
    return await prepareAndWriteAuthBridgeNotificationB0Receipt({
      ...
      withPublicationJournal: operation => {
        ...
        return withB0Journal(publicationContract, operation);
      },
      deploy: async beforeModes => {
        if (interrupted) fail('AUTH_BRIDGE_NOTIFICATION_B0_DEPLOY_INTERRUPTED');
        ...
        const placeholder = authBridgeNotificationB0VersionContract({
        ...
        const bundle = await buildAuthBridgeNotificationB0WranglerMultipart({
        ...
        const contract = authBridgeNotificationB0VersionContract({
          accountId: values.WARPKEEP_AUTH_BRIDGE_ACCOUNT_ID,
          zoneId: values.WARPKEEP_AUTH_BRIDGE_ZONE_ID,
          sourceCommit: values.GITHUB_SHA,
        ...
        if (
          publicationContract !== undefined
          && JSON.stringify(publicationContract) !== JSON.stringify(contract)
        ) fail('AUTH_BRIDGE_NOTIFICATION_B0_DEPLOY_CONTRACT_CHANGED');
        publicationContract = contract;
        try {
          await withB0Journal(contract, async journal => {
...
if (invokedPath === import.meta.url) {
  runAuthBridgeNotificationB0Deploy().then(
    result => {
      process.stdout.write(`AUTH_BRIDGE_NOTIFICATION_B0_DEPLOY_COMPLETE ${result.receiptDigest}\n`);
    },
    error => {
      const code = typeof error?.code === 'string'
        && /^[A-Z0-9_]{8,128}$/u.test(error.code)
        ? error.code
        : 'AUTH_BRIDGE_NOTIFICATION_B0_DEPLOY_FAILED';
