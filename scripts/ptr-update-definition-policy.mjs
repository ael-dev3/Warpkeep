...
export const G002_UPDATE_DEFINITION_POLICY =
  'warpkeep-g002-raw-v10-stable-row-schema-v1';

// Share the exact preservation algorithm while fixing each commitment domain.
function createFixedDefinitionPolicy(profile, errorCode) {
  const fail = () => {
  ...
  function parseUpdateDefinition(serverInnerBytes) {
  ...
  function compareUpdateDefinitions({
    priorServerSchema,
    candidateArtifactDefinition,
  }) {
    const prior = parseUpdateDefinition(priorServerSchema);
  ...
  return Object.freeze({ parseUpdateDefinition, compareUpdateDefinitions });
}

const ptr = createFixedDefinitionPolicy(PTR_UPDATE_DEFINITION_POLICY, 'PTR_UPDATE_DEFINITION_POLICY_INVALID');
const g002 = createFixedDefinitionPolicy(G002_UPDATE_DEFINITION_POLICY, 'G002_UPDATE_DEFINITION_POLICY_INVALID');
export function parsePtrUpdateDefinition(serverInnerBytes) { return ptr.parseUpdateDefinition(...arguments); }
export function comparePtrUpdateDefinitions(input) { return ptr.compareUpdateDefinitions(...arguments); }
export function parseG002UpdateDefinition(serverInnerBytes) { return g002.parseUpdateDefinition(...arguments); }
export function compareG002UpdateDefinitions(input) { return g002.compareUpdateDefinitions(...arguments); }
