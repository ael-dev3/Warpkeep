...
The root [local release assembler](../../scripts/local-release-assembler.mjs)
connects native source capture, fixed compiled producers, complete generated
consumer derivation, independent candidate verification and durable recovery.
Follow its [operating guide](../../docs/operations/0.4.0-local-release-preparation.md)
for `prepare`, `check` and `recover` against the selected committed source.
A prepared source candidate remains non-authorizing; its installation/recovery
proofs are distinct from production database recovery preserving later player
writes, genuine activation evidence and live signer acceptance.
