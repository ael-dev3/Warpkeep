...
type Endpoint = 'status' | 'issue' | 'claim' | 'complete' | 'reconcile' | 'terminal'
...
const KEYS = { status: 'statusJws', issue: 'authorizationJws', claim: 'claimReceiptJws', complete: 'terminalJws', reconcile: 'terminalJws', terminal: 'terminalJws' } as const
const UNAVAILABLE = { unknown: 'RECOVERY_UNAVAILABLE', status: 'RECOVERY_STATUS_UNAVAILABLE', issue: 'RECOVERY_ISSUE_UNAVAILABLE', claim: 'RECOVERY_CLAIM_UNAVAILABLE',
...
async function boundedRpc(operation: () => Promise<unknown>, endpoint: Endpoint): Promise<unknown> {
  ...
  try {
    return await Promise.race([operation(), new Promise<never>((_, reject) => {
      timeout = setTimeout(() => reject(new Error('RECOVERY_RPC_TIMEOUT')), endpoint === 'issue' ? 390000 : endpoint === 'claim' ? 95000 : 20000)
    })])
  ...
  return { async fetch(request: Request): Promise<Response> {
    ...
    try {
      ...
      if (get) {
        ...
        try { parsed = snapshotSignerRequest(endpoint, parseGitHubJsonObject(bytes, 'RECOVERY_REQUEST_INVALID', [])) }
      ...
      const result = await boundedRpc(() => selected === 'status' ? input.signer.status() : input.signer[selected](rpcRequest!), selected)
