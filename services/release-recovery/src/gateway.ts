...
type Endpoint = 'prepare' | 'status' | 'issue' | 'claim' | 'complete' | 'reconcile' | 'terminal'
...
const KEYS = { prepare: 'preparationReceiptJws', status: 'statusJws', issue: 'authorizationJws', claim: 'claimReceiptJws', complete: 'terminalJws', reconcile: 'terminalJws', terminal: 'terminalJws' } as const
const UNAVAILABLE = { prepare: 'RECOVERY_PREPARATION_UNAVAILABLE', unknown: 'RECOVERY_UNAVAILABLE', status: 'RECOVERY_STATUS_UNAVAILABLE', issue: 'RECOVERY_ISSUE_UNAVAILABLE', claim: 'RECOVERY_CLAIM_UNAVAILABLE',
...
async function boundedRpc(operation: () => Promise<unknown>, endpoint: Endpoint): Promise<unknown> {
  ...
  try {
    return await Promise.race([operation(), new Promise<never>((_, reject) => {
      timeout = setTimeout(() => reject(new Error('RECOVERY_RPC_TIMEOUT')), endpoint === 'prepare' ? 110000 : endpoint === 'issue' ? 390000 : endpoint === 'claim' ? 95000 : 20000)
    })])
    ...
    try {
      ...
      if (get) {
        ...
        try {
          ...
          parsed = endpoint === 'prepare' ? snapshotPreparationRequest(value) : snapshotSignerRequest(endpoint, value)
        }
      ...
      const result = await boundedRpc(() => selected === 'status' ? input.signer.status() : selected === 'prepare' ? (input.signer.prepare?.(rpcRequest!) ?? Promise.reject(new Error('RECOVERY_PREPARATION_UNAVAILABLE'))) : input.signer[selected](rpcRequest!), selected)
