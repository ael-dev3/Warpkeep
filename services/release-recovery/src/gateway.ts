...
import { snapshotPtrObservationRequest, snapshotPtrUpdateObservationRequest, type PtrUpdateObservationRequest } from './ptrObservation.js'

type Endpoint = 'ptr-update-observation' | 'ptr-observation' | 'preparation-observation' | 'prepare' | 'status' | 'issue' | 'claim' | 'complete' | 'reconcile' | 'terminal'
...
const KEYS = { 'ptr-update-observation': 'ptrUpdateObservationJws', 'ptr-observation': 'ptrObservationJws', 'preparation-observation': 'preparationObservationJws', prepare: 'preparationReceiptJws', status: 'statusJws', issue: 'authorizationJws', claim: 'claimReceiptJws', complete: 'terminalJws', reconcile: 'terminalJws', terminal: 'terminalJws' } as const
const UNAVAILABLE = { 'ptr-update-observation': 'RECOVERY_PTR_UPDATE_OBSERVATION_UNAVAILABLE', 'ptr-observation': 'RECOVERY_PTR_OBSERVATION_UNAVAILABLE', 'preparation-observation': 'RECOVERY_PREPARATION_OBSERVATION_UNAVAILABLE', prepare: 'RECOVERY_PREPARATION_UNAVAILABLE', unknown: 'RECOVERY_UNAVAILABLE', status: 'RECOVERY_STATUS_UNAVAILABLE', issue: 'RECOVERY_ISSUE_UNAVAILABLE', claim: 'RECOVERY_CLAIM_UNAVAILABLE',
...
async function boundedRpc(operation: () => Promise<unknown>, endpoint: Endpoint): Promise<unknown> {
  ...
  try {
    return await Promise.race([operation(), new Promise<never>((_, reject) => {
      timeout = setTimeout(() => reject(new Error('RECOVERY_RPC_TIMEOUT')), (endpoint === 'prepare' || endpoint === 'preparation-observation' || endpoint === 'ptr-observation' || endpoint === 'ptr-update-observation') ? 110000 : endpoint === 'issue' ? 390000 : endpoint === 'claim' ? 95000 : 20000)
    })])
    ...
    try {
      ...
      if (get) {
        ...
        try {
          ...
          if (endpoint === 'ptr-update-observation') updateRequest = snapshotPtrUpdateObservationRequest(value)
