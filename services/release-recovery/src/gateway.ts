...
import { snapshotPtrObservationRequest, snapshotPtrUpdateObservationRequest, snapshotG002UpdateObservationRequest, type G002UpdateObservationRequest, type PtrUpdateObservationRequest } from './ptrObservation.js'
...
const KEYS = { 'g002-update-observation': 'g002UpdateObservationJws', 'ptr-update-observation': 'ptrUpdateObservationJws', 'ptr-observation': 'ptrObservationJws', 'preparation-observation': 'preparationObservationJws', prepare: 'preparationReceiptJws', status: 'statusJws', issue: 'authorizationJws', claim: 'claimReceiptJws', complete: 'terminalJws', reconcile: 'terminalJws', terminal: 'terminalJws' } as const
const UNAVAILABLE = { 'g002-update-observation': 'RECOVERY_G002_UPDATE_OBSERVATION_UNAVAILABLE', 'ptr-update-observation': 'RECOVERY_PTR_UPDATE_OBSERVATION_UNAVAILABLE', 'ptr-observation': 'RECOVERY_PTR_OBSERVATION_UNAVAILABLE', 'preparation-observation': 'RECOVERY_PREPARATION_OBSERVATION_UNAVAILABLE', prepare: 'RECOVERY_PREPARATION_UNAVAILABLE', unknown: 'RECOVERY_UNAVAILABLE', status: 'RECOVERY_STATUS_UNAVAILABLE', issue: 'RECOVERY_ISSUE_UNAVAILABLE', claim: 'RECOVERY_CLAIM_UNAVAILABLE',
...
async function boundedRpc(operation: () => Promise<unknown>, endpoint: Endpoint): Promise<unknown> {
  ...
  try {
    return await Promise.race([operation(), new Promise<never>((_, reject) => {
      timeout = setTimeout(() => reject(new Error('RECOVERY_RPC_TIMEOUT')), (endpoint === 'prepare' || endpoint === 'preparation-observation' || endpoint === 'ptr-observation' || endpoint === 'ptr-update-observation' || endpoint === 'g002-update-observation') ? 110000 : endpoint === 'issue' ? 390000 : endpoint === 'claim' ? 95000 : 20000)
    })])
    ...
    try {
      ...
      if (get) {
        ...
        try {
          ...
          if (endpoint === 'g002-update-observation') updateRequest = snapshotG002UpdateObservationRequest(value)
          else if (endpoint === 'ptr-update-observation') updateRequest = snapshotPtrUpdateObservationRequest(value)
          else parsed = endpoint === 'ptr-observation' ? snapshotPtrObservationRequest(value)
        ...
        requestId = updateRequest?.requestId ?? parsed?.requestId ?? null
      }
      invoking = true
