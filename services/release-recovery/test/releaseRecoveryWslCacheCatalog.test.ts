...
describe('release recovery WSL v2 cache catalog', () => {
  it('accepts its exact complete tree and rejects one uncatalogued repository file', async () => {
    ...
    const files = [
      ...
      installFile(root, 'source-caches/g001-dependency-closure-sha256.txt', `${'1'.repeat(64)}\n`, 0o400),
      installFile(root, 'source-caches/g002-dependency-closure-sha256.txt', `${'2'.repeat(64)}\n`, 0o400),
      installFile(root, 'source-caches/ptr-dependency-closure-sha256.txt', `${'3'.repeat(64)}\n`, 0o400),
