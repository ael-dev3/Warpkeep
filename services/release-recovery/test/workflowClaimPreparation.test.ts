...
const mocks = vi.hoisted(() => ({ artifact: vi.fn(), preflight: vi.fn(), directory: vi.fn(), begin: vi.fn(), boundary: vi.fn(), finish: vi.fn(), dispose: vi.fn() }));
...
vi.mock('../../../scripts/recovery-workflow-private-directory.mjs', () => ({ createRecoveryWorkflowPrivateDirectory: mocks.directory,
  preflightRecoveryWorkflowPrivateDirectory: mocks.preflight }));
...
beforeEach(() => {
  ...
  mocks.begin.mockImplementation(async (_binding, _context, allocate: () => string) => {
    allocate();
    return { checkDeploymentBoundary: mocks.boundary, finish: mocks.finish, dispose: mocks.dispose };
  });
...
it('uses verified artifact coordinates to allocate storage and prepare one persisted claim', async () => {
  ...
  expect(mocks.begin).toHaveBeenCalledExactlyOnceWith('verified-binding', contextSource, expect.any(Function));
  expect(mocks.artifact.mock.invocationCallOrder[0]).toBeLessThan(mocks.directory.mock.invocationCallOrder[0]!);
  expect(mocks.preflight).toHaveBeenCalledExactlyOnceWith('123', '2');
  expect(mocks.preflight.mock.invocationCallOrder[0]).toBeLessThan(mocks.begin.mock.invocationCallOrder[0]!);
  expect(mocks.begin.mock.invocationCallOrder[0]).toBeLessThan(mocks.directory.mock.invocationCallOrder[0]!);
  expect(mocks.boundary).toHaveBeenCalledTimes(1); expect(mocks.finish).not.toHaveBeenCalled();
  expect(mocks.dispose).toHaveBeenCalledTimes(1);
});
it.each(['artifact', 'preflight'] as const)('does not request authority after %s failure', async boundary => {
